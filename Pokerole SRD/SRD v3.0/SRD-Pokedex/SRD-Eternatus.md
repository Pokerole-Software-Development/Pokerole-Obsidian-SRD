---
Ability1: Pressure
Ability2: ''
AceMoves: []
AdvancedMoves: []
AmateurMoves: []
BaseHP: 7
BeginnerMoves: []
BookSprite: SRD-eternatus-BookSprite.png
DexCategory: "Pok\xE9dex has no data."
DexDescription: "A large meteorite fell on the Galar region 20,000 years ago. Ever\
  \ since this happened, a power surge has been causing Pok\xE9mon to grow gigantic;\
  \ this phenomenon is happening more in recent years."
Dexterity: 7
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Form
  Pokemon: '[[SRD-Eternatus (Eternamax Form)]]'
ExpertMoves: []
GenderType: N
GoodStarter: false
Height:
  Feet: 65.6
  Meters: 20.0
HiddenAbility: ''
HomeSprite: SRD-eternatus-HomeSprite.png
Image: eternatus.png
Insight: 6
Legendary: true
MaxDexterity: 7
MaxInsight: 6
MaxSpecial: 8
MaxStrength: 5
MaxVitality: 6
Name: Eternatus
Number: 890
ProMoves: []
RecommendedRank: Master
RookieMoves: []
Special: 8
StandardMoves: []
StarterMoves: []
Strength: 5
Type1: Poison
Type2: Dragon
Vitality: 6
Weight:
  Kilograms: 950.0
  Pounds: 2094.0
---

#PokeroleSRD/Pokedex

# Eternatus 

> [!grid]
> ![[SRD-eternatus-BookSprite.png|wsmall]]
> ![[SRD-eternatus-HomeSprite.png]]

*Pokédex has no data.*
*A large meteorite fell on the Galar region 20,000 years ago. Ever since this happened, a power surge has been causing Pokémon to grow gigantic; this phenomenon is happening more in recent years.*

**DexID**: 0890
**Name**: Eternatus
**Type**: Poison / Dragon
**Abilities**: [[SRD-Pressure|Pressure]]
**Base HP**: 7

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⬤⬤  | 5/5  |
| Dexterity | ⬤⬤⬤⬤⬤⬤⬤ | 7/7 |
| Vitality  | ⬤⬤⬤⬤⬤⬤  | 6/6  |
| Special   | ⬤⬤⬤⬤⬤⬤⬤⬤   | 8/8   |
| Insight   | ⬤⬤⬤⬤⬤⬤   | 6/6   |

**Height**: 65'6" / 20.0m
**Weight**: 2094.0lbs / 950.0kg
**Good Starter**:: No
**Recommended Rank**:: Master

| Evolves   | Pokemon                            | Kind   |
|:----------|:-----------------------------------|:-------|
| To        | [[SRD-Eternatus (Eternamax Form)]] | Form   |


## Learnset

![[Embedded Views.base#Learnsets]]