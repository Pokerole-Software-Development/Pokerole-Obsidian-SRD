---
Ability1: Static
Ability2: Soundproof
AceMoves:
- Reflect
- Endure
- Zap Cannon
AdvancedMoves:
- Spark
- Charge Beam
- Screech
- Electro Ball
- Light Screen
- Swift
- Self Destruct
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-electrode-BookSprite.png
DexCategory: "Ball Pok\xE9mon"
DexDescription: It is known for causing blackouts in cities. After evolving, it explodes
  as a way to release excess electricity or simply to amuse itself. Trainers need
  to be careful around a bored Electrode.
Dexterity: 4
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Voltorb]]'
  Speed: Slow
ExpertMoves:
- Magnet Rise
- Discharge
- Gyro Ball
- Mirror Coat
- Explosion
GenderType: N
GoodStarter: false
Height:
  Feet: 4.0
  Meters: 1.2
HiddenAbility: Aftermath
HomeSprite: SRD-electrode-HomeSprite.png
Image: electrode.png
Insight: 2
Legendary: false
MaxDexterity: 8
MaxInsight: 5
MaxSpecial: 5
MaxStrength: 4
MaxVitality: 5
Name: Electrode
Number: 101
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Sonic Boom
- Thunder Shock
Special: 2
StandardMoves:
- Eerie Impulse
- Magnetic Flux
- Flash
- Rollout
StarterMoves:
- Charge
- Tackle
Strength: 2
Type1: Electric
Type2: ''
Vitality: 2
Weight:
  Kilograms: 66.0
  Pounds: 146.0
---

#PokeroleSRD/Pokedex

# Electrode 

> [!grid]
> ![[SRD-electrode-BookSprite.png|wsmall]]
> ![[SRD-electrode-HomeSprite.png]]

*Ball Pokémon*
*It is known for causing blackouts in cities. After evolving, it explodes as a way to release excess electricity or simply to amuse itself. Trainers need to be careful around a bored Electrode.*

**DexID**: 0101
**Name**: Electrode
**Type**: Electric
**Abilities**: [[SRD-Static|Static]] / [[SRD-Soundproof|Soundproof]] ([[SRD-Aftermath|Aftermath]])
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⬤⬤⭘⭘⭘⭘ | 4/8 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 4'0" / 1.2m
**Weight**: 146.0lbs / 66.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| From      | [[SRD-Voltorb]] | Level  | Slow    |


## Learnset

![[Embedded Views.base#Learnsets]]