---
Ability1: Synchronize
Ability2: Inner Focus
AceMoves:
- Toxic
- Taunt
- Foul Play
AdvancedMoves:
- Assurance
- Screech
- Snarl
- Mean Look
- Moonlight
- Guard Swap
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-umbreon-BookSprite.png
DexCategory: "Moonlight Pok\xE9mon"
DexDescription: Umbreon evolved from exposure to the moon's energy pulses. It lurks
  in darkness and waits for its foes to move. The rings on its body glow when it leaps
  to attack. Its fur is drenched with poison.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Special
  Pokemon: '[[SRD-Eevee]]'
  Special: Moonlight
  Stat: Happiness
  Value: 4
ExpertMoves:
- Last Resort
- Lash Out
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.3
  Meters: 1.0
HiddenAbility: ''
HomeSprite: SRD-umbreon-HomeSprite.png
Image: umbreon.png
Insight: 3
Legendary: false
MaxDexterity: 4
MaxInsight: 7
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 6
Name: Umbreon
Number: 197
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Tail Whip
- Sand Attack
Special: 2
StandardMoves:
- Pursuit
- Quick Attack
- Confuse Ray
- Feint Attack
StarterMoves:
- Helping Hand
- Tackle
Strength: 2
Type1: Dark
Type2: ''
Vitality: 3
Weight:
  Kilograms: 27.0
  Pounds: 59.0
---

#PokeroleSRD/Pokedex

# Umbreon 

> [!grid]
> ![[SRD-umbreon-BookSprite.png|wsmall]]
> ![[SRD-umbreon-HomeSprite.png]]

*Moonlight Pokémon*
*Umbreon evolved from exposure to the moon's energy pulses. It lurks in darkness and waits for its foes to move. The rings on its body glow when it leaps to attack. Its fur is drenched with poison.*

**DexID**: 0197
**Name**: Umbreon
**Type**: Dark
**Abilities**: [[SRD-Synchronize|Synchronize]] / [[SRD-Inner Focus|Inner Focus]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |

**Height**: 3'3" / 1.0m
**Weight**: 59.0lbs / 27.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon       | Kind    | Stat      |   Value | Special   |
|:----------|:--------------|:--------|:----------|--------:|:----------|
| From      | [[SRD-Eevee]] | Special | Happiness |       4 | Moonlight |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]