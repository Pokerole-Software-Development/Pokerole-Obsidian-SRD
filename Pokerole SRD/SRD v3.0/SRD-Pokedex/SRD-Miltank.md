---
Ability1: Scrappy
Ability2: Thick Fat
AceMoves:
- Helping Hand
- Belch
- Steel Roller
AdvancedMoves:
- Bide
- Body Slam
- Zen Headbutt
- Charm
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-miltank-BookSprite.png
DexCategory: "Milk Cow Pok\xE9mon"
DexDescription: Their male counterpart is Tauros. A Miltank's milk is full of nutrients
  that may heal the sick and the injured. They can produce up to 5 gallons a day.
  Doctors do not recommend milk as treatment for serious injuries.
Dexterity: 3
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Play Rough
- Captivate
- Gyro Ball
- Wake-Up Slap
- Body Press
- High Horsepower
GenderType: F
GoodStarter: false
Height:
  Feet: 4.0
  Meters: 1.2
HiddenAbility: Sap Sipper
HomeSprite: SRD-miltank-HomeSprite.png
Image: miltank.png
Insight: 2
Legendary: false
MaxDexterity: 6
MaxInsight: 5
MaxSpecial: 3
MaxStrength: 5
MaxVitality: 6
Name: Miltank
Number: 241
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Growl
- Defense Curl
- Rollout
Special: 1
StandardMoves:
- Milk Drink
- Stomp
- Heal Bell
- Headbutt
StarterMoves:
- Tackle
Strength: 2
Type1: Normal
Type2: ''
Vitality: 3
Weight:
  Kilograms: 150.0
  Pounds: 331.0
---

#PokeroleSRD/Pokedex

# Miltank 

> [!grid]
> ![[SRD-miltank-BookSprite.png|wsmall]]
> ![[SRD-miltank-HomeSprite.png]]

*Milk Cow Pokémon*
*Their male counterpart is Tauros. A Miltank's milk is full of nutrients that may heal the sick and the injured. They can produce up to 5 gallons a day. Doctors do not recommend milk as treatment for serious injuries.*

**DexID**: 0241
**Name**: Miltank
**Type**: Normal
**Abilities**: [[SRD-Scrappy|Scrappy]] / [[SRD-Thick Fat|Thick Fat]] ([[SRD-Sap Sipper|Sap Sipper]])
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⬤⭘⭘⭘ | 3/6 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 4'0" / 1.2m
**Weight**: 331.0lbs / 150.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets v3.0]]