---
Ability1: Solid Rock
Ability2: Lightning Rod
AceMoves:
- Guard Split
- Wide Guard
- Breaking Swipe
AdvancedMoves:
- Chip Away
- Take Down
- Rock Smash
- High Horsepower
- Drill Run
- Poison Jab
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-rhyperior-BookSprite.png
DexCategory: "Drill Pok\xE9mon"
DexDescription: "They are very scarce in the wild and only live on tall mountains.\
  \ The holes on their hands are used like cannons to shoot boulders and ocasionally\
  \ other Pok\xE9mon. Be careful; they are aggressive, but not very smart."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Item: Protector
  Kind: Trade
  Pokemon: '[[SRD-Rhydon]]'
ExpertMoves:
- Hammer Arm
- Earthquake
- Megahorn
- Stone Edge
- Horn Drill
- Stealth Rock
- Giga Impact
- Rock Wrecker
GenderType: ''
GoodStarter: false
Height:
  Feet: 7.8
  Meters: 2.4
HiddenAbility: Reckless
HomeSprite: SRD-rhyperior-HomeSprite.png
Image: rhyperior.png
Insight: 2
Legendary: false
MaxDexterity: 3
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 7
MaxVitality: 7
Name: Rhyperior
Number: 464
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Stomp
- Fury Attack
Special: 2
StandardMoves:
- Smack Down
- Scary Face
- Rock Blast
- Bulldoze
StarterMoves:
- Horn Attack
- Tail Whip
Strength: 3
Type1: Ground
Type2: Rock
Vitality: 3
Weight:
  Kilograms: 564.0
  Pounds: 1240.0
---

#PokeroleSRD/Pokedex

# Rhyperior 

> [!grid]
> ![[SRD-rhyperior-BookSprite.png|wsmall]]
> ![[SRD-rhyperior-HomeSprite.png]]

*Drill Pokémon*
*They are very scarce in the wild and only live on tall mountains. The holes on their hands are used like cannons to shoot boulders and ocasionally other Pokémon. Be careful; they are aggressive, but not very smart.*

**DexID**: 0464
**Name**: Rhyperior
**Type**: Ground / Rock
**Abilities**: [[SRD-Solid Rock|Solid Rock]] / [[SRD-Lightning Rod|Lightning Rod]] ([[SRD-Reckless|Reckless]])
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 7'8" / 2.4m
**Weight**: 1240.0lbs / 564.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon        | Kind   | Item      |
|:----------|:---------------|:-------|:----------|
| From      | [[SRD-Rhydon]] | Trade  | Protector |


## Learnset

![[Embedded Views.base#Learnsets]]