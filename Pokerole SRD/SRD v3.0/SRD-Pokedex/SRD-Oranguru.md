---
Ability1: Inner Focus
Ability2: Telepathy
AceMoves:
- Expanding Force
- Hyper Beam
- Imprison
AdvancedMoves:
- Zen Headbutt
- Trick Room
- Light Screen
- Reflect
- Psychic
- Psych Up
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-oranguru-BookSprite.png
DexCategory: "Sage Pok\xE9mon"
DexDescription: "This solitary Pok\xE9mon lives deep in the forests where it provides\
  \ food and herbal medicine to those in need. It is incredibly smart, even learning\
  \ how to use Pok\xE9balls. For that reason it dislikes being ordered around."
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Psych Up
- Wonder Room
- Psychic Terrain
- Spite
- Instruct
- Foul Play
- Future Sight
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.0
  Meters: 1.5
HiddenAbility: Symbiosis
HomeSprite: SRD-oranguru-HomeSprite.png
Image: oranguru.png
Insight: 3
Legendary: false
MaxDexterity: 4
MaxInsight: 6
MaxSpecial: 5
MaxStrength: 4
MaxVitality: 5
Name: Oranguru
Number: 765
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- After You
- Feint Attack
Special: 2
StandardMoves:
- Calm Mind
- Stored Power
- Quash
- Nasty Plot
StarterMoves:
- Confusion
- Taunt
Strength: 2
Type1: Normal
Type2: Psychic
Vitality: 2
Weight:
  Kilograms: 76.0
  Pounds: 167.0
---

#PokeroleSRD/Pokedex

# Oranguru 

> [!grid]
> ![[SRD-oranguru-BookSprite.png|wsmall]]
> ![[SRD-oranguru-HomeSprite.png]]

*Sage Pokémon*
*This solitary Pokémon lives deep in the forests where it provides food and herbal medicine to those in need. It is incredibly smart, even learning how to use Pokéballs. For that reason it dislikes being ordered around.*

**DexID**: 0765
**Name**: Oranguru
**Type**: Normal / Psychic
**Abilities**: [[SRD-Inner Focus|Inner Focus]] / [[SRD-Telepathy|Telepathy]] ([[SRD-Symbiosis|Symbiosis]])
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 5'0" / 1.5m
**Weight**: 167.0lbs / 76.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets v3.0]]