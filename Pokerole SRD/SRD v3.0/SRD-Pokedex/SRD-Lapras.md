---
Ability1: Shell Armor
Ability2: Water Absorb
AceMoves:
- Dragon Dance
- Zap Cannon
- Outrage
AdvancedMoves:
- Water Pulse
- Brine
- Rain Dance
- Ice Beam
- Safeguard
- Sparkling Aria
AmateurMoves: []
BaseHP: 7
BeginnerMoves: []
BookSprite: SRD-lapras-BookSprite.png
DexCategory: "Transport Pok\xE9mon"
DexDescription: "Humans drove Lapras close to extinction. In the evenings, this Pok\xE9\
  mon is said to sing as it seeks what few others of its kind still remain. Their\
  \ gentle nature has made them easy to lure and catch."
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Freeze Dry
- Echoed Voice
- Hydro Pump
- Perish Song
- Sheer Cold
GenderType: ''
GoodStarter: false
Height:
  Feet: 8.2
  Meters: 2.5
HiddenAbility: Hydration
HomeSprite: SRD-lapras-HomeSprite.png
Image: lapras.png
Insight: 3
Legendary: false
MaxDexterity: 4
MaxInsight: 6
MaxSpecial: 5
MaxStrength: 5
MaxVitality: 5
Name: Lapras
Number: 131
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Water Gun
- Mist
Special: 2
StandardMoves:
- Life Dew
- Ice Shard
- Confuse Ray
- Body Slam
StarterMoves:
- Sing
- Growl
Strength: 2
Type1: Water
Type2: Ice
Vitality: 2
Weight:
  Kilograms: 132.0
  Pounds: 292.0
---

#PokeroleSRD/Pokedex

# Lapras 

> [!grid]
> ![[SRD-lapras-BookSprite.png|wsmall]]
> ![[SRD-lapras-HomeSprite.png]]

*Transport Pokémon*
*Humans drove Lapras close to extinction. In the evenings, this Pokémon is said to sing as it seeks what few others of its kind still remain. Their gentle nature has made them easy to lure and catch.*

**DexID**: 0131
**Name**: Lapras
**Type**: Water / Ice
**Abilities**: [[SRD-Shell Armor|Shell Armor]] / [[SRD-Water Absorb|Water Absorb]] ([[SRD-Hydration|Hydration]])
**Base HP**: 7

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 8'2" / 2.5m
**Weight**: 292.0lbs / 132.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets v3.0]]