---
Ability1: Ice Face
Ability2: ''
AceMoves:
- Liquidation
- Belly Drum
- Substitute
AdvancedMoves:
- Avalanche
- Soak
- Amnesia
- Hail
- Freeze Dry
- Aurora Veil
- Whirlpool
- Icicle Crash
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-eiscue-no-ice-form-BookSprite.png
DexCategory: Penguin Pokemon
DexDescription: Eiscue (No-Ice) Form is actually its true Form. Without the ice to
  protect it you can see its worried expression, it's actually quite endearing. It
  can now move faster but you can see it feels anxious.
Dexterity: 3
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Form
  Pokemon: '[[SRD-Eiscue]]'
ExpertMoves:
- Iron Defense
- Agility
- Surf
- Aqua Ring
- Head Smash
- Blizzard
GenderType: ''
GoodStarter: false
Height:
  Feet: 4.6
  Meters: 1.4
HiddenAbility: ''
HomeSprite: SRD-eiscue-no-ice-form-HomeSprite.png
Image: eiscue-no-ice-form.png
Insight: 2
Legendary: false
MaxDexterity: 7
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 5
MaxVitality: 5
Name: Eiscue (No Ice Form)
Number: 875
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Powder Snow
- Icy Wind
Special: 2
StandardMoves:
- Weather Ball
- Headbutt
- Iron Head
- Protect
StarterMoves:
- Tackle
- Mist
Strength: 2
Type1: Ice
Type2: ''
Vitality: 2
Weight:
  Kilograms: 89.0
  Pounds: 196.0
---

#PokeroleSRD/Pokedex

# Eiscue (No Ice Form) 

> [!grid]
> ![[SRD-eiscue-no-ice-form-BookSprite.png|wsmall]]
> ![[SRD-eiscue-no-ice-form-HomeSprite.png]]

*Penguin Pokemon*
*Eiscue (No-Ice) Form is actually its true Form. Without the ice to protect it you can see its worried expression, it's actually quite endearing. It can now move faster but you can see it feels anxious.*

**DexID**: 0875F1
**Name**: Eiscue (No Ice Form)
**Type**: Ice
**Abilities**: [[SRD-Ice Face|Ice Face]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⬤⭘⭘⭘⭘ | 3/7 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 4'6" / 1.4m
**Weight**: 196.0lbs / 89.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon        | Kind   |
|:----------|:---------------|:-------|
| From      | [[SRD-Eiscue]] | Form   |


## Learnset

![[Embedded Views.base#Learnsets]]