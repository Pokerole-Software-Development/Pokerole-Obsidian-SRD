---
Ability1: Torrent
Ability2: Shell Armor
AceMoves:
- Dive
- Detect
- Water Pledge
AdvancedMoves:
- Revenge
- Aqua Jet
- Encore
- Aqua Tail
- Cut
- X-Scissor
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-dewott-BookSprite.png
DexCategory: "Discipline Pok\xE9mon"
DexDescription: "It isolates itself from others and trains every day to perform a\
  \ double scalchop slash technique. This Pok\xE9mon takes itself very seriously and\
  \ won't back down from any challenge."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Oshawott]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Samurott]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Samurott (Hisuian Form)]]'
  Region: Hisui
  Speed: Medium
ExpertMoves:
- Retaliate
- Swords Dance
- Taunt
- Hydro Pump
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: ''
HomeSprite: SRD-dewott-HomeSprite.png
Image: dewott.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 5
MaxVitality: 4
Name: Dewott
Number: 502
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Water Gun
- Water Sport
Special: 2
StandardMoves:
- Focus Energy
- Razor Shell
- Fury Cutter
- Water Pulse
StarterMoves:
- Tackle
- Tail Whip
Strength: 2
Type1: Water
Type2: ''
Vitality: 2
Weight:
  Kilograms: 24.0
  Pounds: 54.0
---

#PokeroleSRD/Pokedex

# Dewott 

> [!grid]
> ![[SRD-dewott-BookSprite.png|wsmall]]
> ![[SRD-dewott-HomeSprite.png]]

*Discipline Pokémon*
*It isolates itself from others and trains every day to perform a double scalchop slash technique. This Pokémon takes itself very seriously and won't back down from any challenge.*

**DexID**: 0502
**Name**: Dewott
**Type**: Water
**Abilities**: [[SRD-Torrent|Torrent]] / [[SRD-Shell Armor|Shell Armor]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'6" / 0.8m
**Weight**: 54.0lbs / 24.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon                         | Kind   | Speed   | Region   |
|:----------|:--------------------------------|:-------|:--------|:---------|
| From      | [[SRD-Oshawott]]                | Level  | Medium  |          |
| To        | [[SRD-Samurott]]                | Level  | Medium  |          |
| To        | [[SRD-Samurott (Hisuian Form)]] | Level  | Medium  | Hisui    |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]