---
Ability1: Run Away
Ability2: Stakeout
AceMoves:
- Torment
- Pursuit
- Baton Pass
AdvancedMoves:
- Nasty Plot
- Sucker Punch
- Agility
- Dig
- Night Slash
- Tail Slap
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-nickit-BookSprite.png
DexCategory: "Fox Pok\xE9mon"
DexDescription: Cunning and cautious, Nickit survive by stealing food. It erases its
  tracks with swipes of its tail so it's very difficult to track one. They have thrived
  in the cities and come out at night to steal from stores.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Thievul]]'
  Speed: Medium
ExpertMoves:
- Foul Play
GenderType: ''
GoodStarter: true
Height:
  Feet: 2.0
  Meters: 0.6
HiddenAbility: ''
HomeSprite: SRD-nickit-HomeSprite.png
Image: nickit.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 3
MaxVitality: 3
Name: Nickit
Number: 827
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Beat Up
- Hone Claws
Special: 2
StandardMoves:
- Snarl
- Assurance
- Fake Tears
- Play Rough
- Quick Guard
- Knock Off
StarterMoves:
- Quick Attack
- Tail Whip
Strength: 1
Type1: Dark
Type2: ''
Vitality: 1
Weight:
  Kilograms: 8.9
  Pounds: 19.6
---

#PokeroleSRD/Pokedex

# Nickit 

> [!grid]
> ![[SRD-nickit-BookSprite.png|wsmall]]
> ![[SRD-nickit-HomeSprite.png]]

*Fox Pokémon*
*Cunning and cautious, Nickit survive by stealing food. It erases its tracks with swipes of its tail so it's very difficult to track one. They have thrived in the cities and come out at night to steal from stores.*

**DexID**: 0827
**Name**: Nickit
**Type**: Dark
**Abilities**: [[SRD-Run Away|Run Away]] / [[SRD-Stakeout|Stakeout]]
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⭘⭘  | 1/3  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'0" / 0.6m
**Weight**: 19.6lbs / 8.9kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Thievul]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]