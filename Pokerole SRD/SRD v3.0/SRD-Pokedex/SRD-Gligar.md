---
Ability1: Hyper Cutter
Ability2: Sand Veil
AceMoves:
- Iron Tail
- Poison Tail
- Cut
AdvancedMoves:
- Slash
- Sky Uppercut
- Screech
- Swords Dance
- X-Scissor
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-gligar-BookSprite.png
DexCategory: "Fly Scorpion Pok\xE9mon"
DexDescription: It hangs from cliffs waiting for its prey. It flies straight at the
  target's face, grapples the victim with its pincers and clawed hind legs, and lastly,
  it injects poison with its tail. Gligar can glide without making any noise.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Item: Razor Fang
  Kind: Level
  Pokemon: '[[SRD-Gliscor]]'
ExpertMoves:
- Earthquake
- Guillotine
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.6
  Meters: 1.1
HiddenAbility: Immunity
HomeSprite: SRD-gligar-HomeSprite.png
Image: gligar.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 3
MaxStrength: 5
MaxVitality: 6
Name: Gligar
Number: 207
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Harden
- Knock Off
Special: 1
StandardMoves:
- Quick Attack
- Fury Cutter
- Mud Slap
- Acrobatics
- Feint Attack
- U-Turn
StarterMoves:
- Poison Sting
- Sand Attack
Strength: 2
Type1: Ground
Type2: Flying
Vitality: 3
Weight:
  Kilograms: 65.0
  Pounds: 143.0
---

#PokeroleSRD/Pokedex

# Gligar 

> [!grid]
> ![[SRD-gligar-BookSprite.png|wsmall]]
> ![[SRD-gligar-HomeSprite.png]]

*Fly Scorpion Pokémon*
*It hangs from cliffs waiting for its prey. It flies straight at the target's face, grapples the victim with its pincers and clawed hind legs, and lastly, it injects poison with its tail. Gligar can glide without making any noise.*

**DexID**: 0207
**Name**: Gligar
**Type**: Ground / Flying
**Abilities**: [[SRD-Hyper Cutter|Hyper Cutter]] / [[SRD-Sand Veil|Sand Veil]] ([[SRD-Immunity|Immunity]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 3'6" / 1.1m
**Weight**: 143.0lbs / 65.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon         | Kind   | Item       |
|:----------|:----------------|:-------|:-----------|
| To        | [[SRD-Gliscor]] | Level  | Razor Fang |


## Learnset

![[Embedded Views.base#Learnsets]]