---
Ability1: Galvanize
Ability2: Sturdy
AceMoves:
- Counter
- Magnet Rise
- Wild Charge
AdvancedMoves:
- Smack Down
- Rock Blast
- Thunder Punch
- Self Destruct
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-golem-alolan-form-BookSprite.png
DexCategory: "Megaton Pok\xE9mon"
DexDescription: The Alolan variant of Golem does not roll around like its counterpart;
  instead it loads rocks into the charger on its head and shoots them around. If the
  rocks miss, the electric shockwave doesn't.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Trade
  Pokemon: '[[SRD-Graveler (Alolan Form)]]'
ExpertMoves:
- Discharge
- Stealth Rock
- Double-Edge
- Explosion
- Heavy Slam
- Stone Edge
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.6
  Meters: 1.7
HiddenAbility: ''
HomeSprite: SRD-golem-alolan-form-HomeSprite.png
Image: golem-alolan-form.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 8
MaxVitality: 7
Name: Golem (Alolan Form)
Number: 76
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Charge
- Rock Polish
Special: 2
StandardMoves:
- Steamroller
- Sandstorm
- Rock Throw
- Spark
StarterMoves:
- Tackle
- Defense Curl
Strength: 4
Type1: Rock
Type2: Electric
Vitality: 3
Weight:
  Kilograms: 316.0
  Pounds: 696.0
---

#PokeroleSRD/Pokedex

# Golem (Alolan Form) 

> [!grid]
> ![[SRD-golem-alolan-form-BookSprite.png|wsmall]]
> ![[SRD-golem-alolan-form-HomeSprite.png]]

*Megaton Pokémon*
*The Alolan variant of Golem does not roll around like its counterpart; instead it loads rocks into the charger on its head and shoots them around. If the rocks miss, the electric shockwave doesn't.*

**DexID**: 0076A
**Name**: Golem (Alolan Form)
**Type**: Rock / Electric
**Abilities**: [[SRD-Galvanize|Galvanize]] / [[SRD-Sturdy|Sturdy]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⬤⭘⭘⭘⭘  | 4/8  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 5'6" / 1.7m
**Weight**: 696.0lbs / 316.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon                        | Kind   |
|:----------|:-------------------------------|:-------|
| From      | [[SRD-Graveler (Alolan Form)]] | Trade  |


## Learnset

![[Embedded Views.base#Learnsets]]