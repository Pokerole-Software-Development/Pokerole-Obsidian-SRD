---
Ability1: Chlorophyll
Ability2: Leaf Guard
AceMoves:
- Aromatherapy
- Seed Bomb
- Sunny Day
AdvancedMoves:
- Mega Drain
- Leech Seed
- Rage Powder
- Acrobatics
- Cotton Spore
- Worry Seed
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-skiploom-BookSprite.png
DexCategory: "Cottonweed Pok\xE9mon"
DexDescription: "It blooms when the weather is warm. If the weather gets cold, the\
  \ bloom will close and it will stop flying. This is not an aggressive Pok\xE9mon,\
  \ but it can cause allergies if it floats directly above you."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Hoppip]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Jumpluff]]'
  Speed: Medium
ExpertMoves:
- U-Turn
- Bounce
- Silver Wind
- Giga Drain
- Memento
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.0
  Meters: 0.6
HiddenAbility: Infiltrator
HomeSprite: SRD-skiploom-HomeSprite.png
Image: skiploom.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 4
Name: Skiploom
Number: 188
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Tail Whip
- Tackle
Special: 2
StandardMoves:
- Fairy Wind
- Synthesis
- Stun Spore
- Poison Powder
- Sleep Powder
- Bullet Seed
StarterMoves:
- Splash
- Absorb
Strength: 2
Type1: Grass
Type2: Flying
Vitality: 2
Weight:
  Kilograms: 1.0
  Pounds: 2.0
---

#PokeroleSRD/Pokedex

# Skiploom 

> [!grid]
> ![[SRD-skiploom-BookSprite.png|wsmall]]
> ![[SRD-skiploom-HomeSprite.png]]

*Cottonweed Pokémon*
*It blooms when the weather is warm. If the weather gets cold, the bloom will close and it will stop flying. This is not an aggressive Pokémon, but it can cause allergies if it floats directly above you.*

**DexID**: 0188
**Name**: Skiploom
**Type**: Grass / Flying
**Abilities**: [[SRD-Chlorophyll|Chlorophyll]] / [[SRD-Leaf Guard|Leaf Guard]] ([[SRD-Infiltrator|Infiltrator]])
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'0" / 0.6m
**Weight**: 2.0lbs / 1.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon          | Kind   | Speed   |
|:----------|:-----------------|:-------|:--------|
| From      | [[SRD-Hoppip]]   | Level  | Medium  |
| To        | [[SRD-Jumpluff]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]