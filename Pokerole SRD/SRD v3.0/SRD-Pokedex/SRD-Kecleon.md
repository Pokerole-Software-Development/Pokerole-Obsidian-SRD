---
Ability1: Color Change
Ability2: Protean
AceMoves:
- Snatch
- Fake Out
- Trick
AdvancedMoves:
- Disable
- Psybeam
- Ancient Power
- Slash
- Detect
- Camouflage
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-kecleon-BookSprite.png
DexCategory: "Color Swap Pok\xE9mon"
DexDescription: They are able to change their colors to blend with their surroundings.
  The only part of its body that can't change is the red zigzag line on its belly.
  Kecleon is very sneaky, smart and kind of insolent.
Dexterity: 1
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Shadow Claw
- Screech
- Substitute
- Sucker Punch
- Synchronoise
- Foul Play
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.3
  Meters: 1.0
HiddenAbility: ''
HomeSprite: SRD-kecleon-HomeSprite.png
Image: kecleon.png
Insight: 3
Legendary: false
MaxDexterity: 3
MaxInsight: 7
MaxSpecial: 4
MaxStrength: 5
MaxVitality: 5
Name: Kecleon
Number: 352
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Astonish
- Lick
Special: 2
StandardMoves:
- Scratch
- Bind
- Feint
- Shadow Sneak
- Fury Swipes
- Feint Attack
StarterMoves:
- Tail Whip
- Thief
Strength: 2
Type1: Normal
Type2: ''
Vitality: 2
Weight:
  Kilograms: 22.0
  Pounds: 48.0
---

#PokeroleSRD/Pokedex

# Kecleon 

> [!grid]
> ![[SRD-kecleon-BookSprite.png|wsmall]]
> ![[SRD-kecleon-HomeSprite.png]]

*Color Swap Pokémon*
*They are able to change their colors to blend with their surroundings. The only part of its body that can't change is the red zigzag line on its belly. Kecleon is very sneaky, smart and kind of insolent.*

**DexID**: 0352
**Name**: Kecleon
**Type**: Normal
**Abilities**: [[SRD-Color Change|Color Change]] / [[SRD-Protean|Protean]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |

**Height**: 3'3" / 1.0m
**Weight**: 48.0lbs / 22.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard


## Learnset

![[Embedded Views.base#Learnsets]]