---
Ability1: Pickup
Ability2: Run Away
AceMoves:
- Fake Out
- Beat Up
- Bounce
AdvancedMoves:
- Agility
- Fling
- Double Hit
- Nasty Plot
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-aipom-BookSprite.png
DexCategory: "Long Tail Pok\xE9mon"
DexDescription: "It lives at the top of tall trees in forests and jungles. It uses\
  \ its tail tip as a third hand. However, because the Pok\xE9mon uses its tail so\
  \ much, its real hands become rather clumsy. It is very playful."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Special
  Pokemon: '[[SRD-Ambipom]]'
  Special: Learning Double Hit
ExpertMoves:
- Last Resort
GenderType: ''
GoodStarter: true
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: Skill Link
HomeSprite: SRD-aipom-HomeSprite.png
Image: aipom.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 3
MaxStrength: 5
MaxVitality: 4
Name: Aipom
Number: 190
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Sand Attack
- Astonish
Special: 1
StandardMoves:
- Baton Pass
- Tickle
- Fury Swipes
- Swift
- Screech
StarterMoves:
- Scratch
- Tail Whip
Strength: 2
Type1: Normal
Type2: ''
Vitality: 2
Weight:
  Kilograms: 11.0
  Pounds: 25.0
---

#PokeroleSRD/Pokedex

# Aipom 

> [!grid]
> ![[SRD-aipom-BookSprite.png|wsmall]]
> ![[SRD-aipom-HomeSprite.png]]

*Long Tail Pokémon*
*It lives at the top of tall trees in forests and jungles. It uses its tail tip as a third hand. However, because the Pokémon uses its tail so much, its real hands become rather clumsy. It is very playful.*

**DexID**: 0190
**Name**: Aipom
**Type**: Normal
**Abilities**: [[SRD-Pickup|Pickup]] / [[SRD-Run Away|Run Away]] ([[SRD-Skill Link|Skill Link]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'6" / 0.8m
**Weight**: 25.0lbs / 11.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon         | Kind    | Special             |
|:----------|:----------------|:--------|:--------------------|
| To        | [[SRD-Ambipom]] | Special | Learning Double Hit |


## Learnset

![[Embedded Views.base#Learnsets]]