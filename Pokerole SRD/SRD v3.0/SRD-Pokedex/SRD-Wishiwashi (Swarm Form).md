---
Ability1: Schooling
Ability2: ''
AceMoves:
- Muddy Water
- Water Pulse
- Mist
AdvancedMoves:
- Uproar
- Aqua Tail
- Aqua Ring
- Take Down
- Beat Up
- Flip Turn
AmateurMoves: []
BaseHP: 8
BeginnerMoves: []
BookSprite: SRD-wishiwashi-swarm-form-BookSprite.png
DexCategory: "Small Fry Pok\xE9mon"
DexDescription: A school of Wishiwashi is the demon of the sea. Even Gyarados flee
  in terror when one approaches. Since it is formed by lots of members, it's impossible
  to control. They disband after the threat is 'neutralized'.
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Form
  Pokemon: '[[SRD-Wishiwashi]]'
ExpertMoves:
- Whirlpool
- Endeavor
- Double-Edge
- Hydro Pump
GenderType: ''
GoodStarter: false
Height:
  Feet: 27.0
  Meters: 8.2
HiddenAbility: ''
HomeSprite: SRD-wishiwashi-swarm-form-HomeSprite.png
Image: wishiwashi-swarm-form.png
Insight: 3
Legendary: false
MaxDexterity: 3
MaxInsight: 7
MaxSpecial: 7
MaxStrength: 7
MaxVitality: 7
Name: Wishiwashi (Swarm Form)
Number: 746
ProMoves: []
RecommendedRank: Ace
RookieMoves:
- Helping Hand
- Feint Attack
Special: 3
StandardMoves:
- Brine
- Tearful Look
- Dive
- Soak
StarterMoves:
- Water Gun
- Growl
- Liquidation
- Protect
- Earthquake
Strength: 3
Type1: Water
Type2: ''
Vitality: 3
Weight:
  Kilograms: 234.0
  Pounds: 515.0
---

#PokeroleSRD/Pokedex

# Wishiwashi (Swarm Form) 

> [!grid]
> ![[SRD-wishiwashi-swarm-form-BookSprite.png|wsmall]]
> ![[SRD-wishiwashi-swarm-form-HomeSprite.png]]

*Small Fry Pokémon*
*A school of Wishiwashi is the demon of the sea. Even Gyarados flee in terror when one approaches. Since it is formed by lots of members, it's impossible to control. They disband after the threat is 'neutralized'.*

**DexID**: 0746F1
**Name**: Wishiwashi (Swarm Form)
**Type**: Water
**Abilities**: [[SRD-Schooling|Schooling]]
**Base HP**: 8

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Special   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |
| Insight   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |

**Height**: 27'0" / 8.2m
**Weight**: 515.0lbs / 234.0kg
**Good Starter**:: No
**Recommended Rank**:: Ace

| Evolves   | Pokemon            | Kind   |
|:----------|:-------------------|:-------|
| From      | [[SRD-Wishiwashi]] | Form   |


## Learnset

![[Embedded Views.base#Learnsets]]