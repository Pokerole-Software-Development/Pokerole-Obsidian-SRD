---
Ability1: Leaf Guard
Ability2: Sweet Veil
AceMoves:
- Charm
- Bounce
- Grassy Glide
AdvancedMoves:
- Flail
- Aromatic Mist
- Aromatherapy
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-bounsweet-BookSprite.png
DexCategory: "Fruit Pok\xE9mon"
DexDescription: "This happy-go-lucky Pok\xE9mon produces a delicious aroma that unfortunately\
  \ attracts predators to it. When it escapes, it looks as if it's happily skipping\
  \ around so it rarely gets the help it needs."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Steenee]]'
  Speed: Medium
ExpertMoves:
- Play Rough
GenderType: F
GoodStarter: true
Height:
  Feet: 1.0
  Meters: 0.3
HiddenAbility: Oblivious
HomeSprite: SRD-bounsweet-HomeSprite.png
Image: bounsweet.png
Insight: 1
Legendary: false
MaxDexterity: 3
MaxInsight: 3
MaxSpecial: 3
MaxStrength: 3
MaxVitality: 3
Name: Bounsweet
Number: 761
ProMoves: []
RecommendedRank: Starter
RookieMoves:
- Rapid Spin
- Razor Leaf
Special: 1
StandardMoves:
- Sweet Scent
- Magical Leaf
- Teeter Dance
StarterMoves:
- Splash
- Play Nice
Strength: 1
Type1: Grass
Type2: ''
Vitality: 1
Weight:
  Kilograms: 3.0
  Pounds: 7.0
---

#PokeroleSRD/Pokedex

# Bounsweet 

> [!grid]
> ![[SRD-bounsweet-BookSprite.png|wsmall]]
> ![[SRD-bounsweet-HomeSprite.png]]

*Fruit Pokémon*
*This happy-go-lucky Pokémon produces a delicious aroma that unfortunately attracts predators to it. When it escapes, it looks as if it's happily skipping around so it rarely gets the help it needs.*

**DexID**: 0761
**Name**: Bounsweet
**Type**: Grass
**Abilities**: [[SRD-Leaf Guard|Leaf Guard]] / [[SRD-Sweet Veil|Sweet Veil]] ([[SRD-Oblivious|Oblivious]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⭘⭘  | 1/3  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⭘⭘   | 1/3   |

**Height**: 1'0" / 0.3m
**Weight**: 7.0lbs / 3.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Starter

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Steenee]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]