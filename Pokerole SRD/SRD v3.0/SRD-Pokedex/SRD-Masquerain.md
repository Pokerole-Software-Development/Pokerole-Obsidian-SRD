---
Ability1: Swift Swim
Ability2: Intimidate
AceMoves:
- Ominous Wind
- Giga Drain
- Hydro Pump
AdvancedMoves:
- Agility
- Haze
- Mist
- Sticky Web
- Air Cutter
- Stun Spore
- Whirlwind
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-masquerain-BookSprite.png
DexCategory: "Eyeball Pok\xE9mon"
DexDescription: Their antennas look like terrifying eyes. Masquerain can fly in any
  direction like a helicopter, but their wings are soft and fragile, so they don't
  fly when it's raining. They cling to trees at night to sleep.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Surskit]]'
  Speed: Fast
ExpertMoves:
- Bubble Beam
- Aqua Jet
- Baton Pass
- Bug Buzz
- Air Slash
- Quiver Dance
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: Unnerve
HomeSprite: SRD-masquerain-HomeSprite.png
Image: masquerain.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 5
MaxSpecial: 6
MaxStrength: 4
MaxVitality: 4
Name: Masquerain
Number: 284
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Sweet Scent
- Bubble
Special: 3
StandardMoves:
- Soak
- Supersonic
- Gust
- Scary Face
StarterMoves:
- Water Gun
- Quick Attack
Strength: 2
Type1: Bug
Type2: Flying
Vitality: 2
Weight:
  Kilograms: 3.0
  Pounds: 8.0
---

#PokeroleSRD/Pokedex

# Masquerain 

> [!grid]
> ![[SRD-masquerain-BookSprite.png|wsmall]]
> ![[SRD-masquerain-HomeSprite.png]]

*Eyeball Pokémon*
*Their antennas look like terrifying eyes. Masquerain can fly in any direction like a helicopter, but their wings are soft and fragile, so they don't fly when it's raining. They cling to trees at night to sleep.*

**DexID**: 0284
**Name**: Masquerain
**Type**: Bug / Flying
**Abilities**: [[SRD-Swift Swim|Swift Swim]] / [[SRD-Intimidate|Intimidate]] ([[SRD-Unnerve|Unnerve]])
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 2'6" / 0.8m
**Weight**: 8.0lbs / 3.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| From      | [[SRD-Surskit]] | Level  | Fast    |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]