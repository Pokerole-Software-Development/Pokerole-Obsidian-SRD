---
Ability1: Good as Gold
Ability2: ''
AceMoves:
- Thunder Wave
- Hyper Beam
- Steel Beam
AdvancedMoves:
- Thief
- Light Screen
- Reflect
- Dazzling Gleam
- Shadow Ball
- Power Gem
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-gholdengo-BookSprite.png
DexCategory: "Coin Entity Pok\xE9mon"
DexDescription: "Its sturdy body and surfing board is made of pure gold. This Pok\xE9\
  mon gets along well with others and is quick to make friends. Loose change is left\
  \ behind wherever Gholdengo has engaged in combat."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Special
  Pokemon: '[[SRD-Gimmighoul (Chest Form)]]'
  Special: Fill the gold chest
ExpertMoves:
- Recover
- Make it Rain
- Nasty Plot
- Memento
GenderType: N
GoodStarter: false
Height:
  Feet: 3.9
  Meters: 1.2
HiddenAbility: ''
HomeSprite: SRD-gholdengo-HomeSprite.png
Image: gholdengo.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 5
MaxSpecial: 7
MaxStrength: 4
MaxVitality: 5
Name: Gholdengo
Number: 1000
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Protect
- Night Shade
Special: 3
StandardMoves:
- Confuse Ray
- Substitute
- Metal Sound
- Hex
StarterMoves:
- Astonish
- Tackle
Strength: 2
Type1: Steel
Type2: Ghost
Vitality: 2
Weight:
  Kilograms: 30.0
  Pounds: 66.0
---

#PokeroleSRD/Pokedex

# Gholdengo 

> [!grid]
> ![[SRD-gholdengo-BookSprite.png|wsmall]]
> ![[SRD-gholdengo-HomeSprite.png]]

*Coin Entity Pokémon*
*Its sturdy body and surfing board is made of pure gold. This Pokémon gets along well with others and is quick to make friends. Loose change is left behind wherever Gholdengo has engaged in combat.*

**DexID**: 1000
**Name**: Gholdengo
**Type**: Steel / Ghost
**Abilities**: [[SRD-Good as Gold|Good as Gold]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 3'9" / 1.2m
**Weight**: 66.0lbs / 30.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon                         | Kind    | Special             |
|:----------|:--------------------------------|:--------|:--------------------|
| From      | [[SRD-Gimmighoul (Chest Form)]] | Special | Fill the gold chest |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]