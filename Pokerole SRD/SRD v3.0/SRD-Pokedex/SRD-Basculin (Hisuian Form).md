---
Ability1: Rattled
Ability2: Adaptability
AceMoves:
- Agility
- Flip Turn
- Last Respects
AdvancedMoves:
- Uproar
- Crunch
- Take Down
- Dive
- Chip Away
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-basculin-hisuian-form-BookSprite.png
DexCategory: "Hostile (?) Pok\xE9mon"
DexDescription: A Basculin variant that existed in old Hisui. It had a timid and skittish
  demeanor, unlike the modern specimens who are prone to violence. It was rare to
  see; it fled as soon as it noticed you. None have been seen in 200 years.
Dexterity: 3
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Basculegion F]]'
  Special: Female
  Speed: Slow
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Basculegion M]]'
  Special: Male
  Speed: Slow
ExpertMoves:
- Endeavor
- Wave Crash
- Double-Edge
- Head Smash
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: Mold Breaker
HomeSprite: SRD-basculin-hisuian-form-HomeSprite.png
Image: basculin-hisuian-form.png
Insight: 2
Legendary: false
MaxDexterity: 6
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 5
MaxVitality: 4
Name: Basculin (Hisuian Form)
Number: 550
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Tackle
- Bite
Special: 2
StandardMoves:
- Flail
- Aqua Jet
- Scary Face
- Headbutt
- Soak
StarterMoves:
- Water Gun
- Tail Whip
Strength: 2
Type1: Water
Type2: ''
Vitality: 2
Weight:
  Kilograms: 11.0
  Pounds: 25.0
---

#PokeroleSRD/Pokedex

# Basculin (Hisuian Form) 

> [!grid]
> ![[SRD-basculin-hisuian-form-BookSprite.png|wsmall]]
> ![[SRD-basculin-hisuian-form-HomeSprite.png]]

*Hostile (?) Pokémon*
*A Basculin variant that existed in old Hisui. It had a timid and skittish demeanor, unlike the modern specimens who are prone to violence. It was rare to see; it fled as soon as it noticed you. None have been seen in 200 years.*

**DexID**: 0550H
**Name**: Basculin (Hisuian Form)
**Type**: Water
**Abilities**: [[SRD-Rattled|Rattled]] / [[SRD-Adaptability|Adaptability]] ([[SRD-Mold Breaker|Mold Breaker]])
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⬤⭘⭘⭘ | 3/6 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'6" / 0.8m
**Weight**: 25.0lbs / 11.0kg
**Good Starter**:: No
**Recommended Rank**:: Rookie

| Evolves   | Pokemon               | Kind   | Speed   | Special   |
|:----------|:----------------------|:-------|:--------|:----------|
| To        | [[SRD-Basculegion F]] | Level  | Slow    | Female    |
| To        | [[SRD-Basculegion M]] | Level  | Slow    | Male      |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]