---
Ability1: Chlorophyll
Ability2: Regenerator
AceMoves:
- Nature Power
- Substitute
- Rage Powder
AdvancedMoves:
- Double Hit
- Acid Spray
- Growth
- Mega Drain
- Knock Off
- Stun Spore
- Natural Gift
- Slam
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-tangrowth-BookSprite.png
DexCategory: "Vine Pok\xE9mon"
DexDescription: While it remains still, it appears to be a large shrub. Unsuspecting
  prey get ensnared by its vines. In the summer its vines grow so large that you can't
  even see its eyes. Its arms regrow even if you cut them.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Tangela]]'
  Speed: Slow
ExpertMoves:
- Ancient Power
- Giga Drain
- Tickle
- Wring Out
- Sludge Bomb
- Brutal Swing
- Grassy Terrain
- Power Whip
GenderType: ''
GoodStarter: false
Height:
  Feet: 6.6
  Meters: 2.0
HiddenAbility: ''
HomeSprite: SRD-tangrowth-HomeSprite.png
Image: tangrowth.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 6
MaxStrength: 6
MaxVitality: 7
Name: Tangrowth
Number: 465
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Absorb
- Vine Whip
Special: 3
StandardMoves:
- Block
- Sleep Powder
- Poison Powder
- Bind
StarterMoves:
- Constrict
- Ingrain
Strength: 3
Type1: Grass
Type2: ''
Vitality: 3
Weight:
  Kilograms: 256.0
  Pounds: 563.0
---

#PokeroleSRD/Pokedex

# Tangrowth 

> [!grid]
> ![[SRD-tangrowth-BookSprite.png|wsmall]]
> ![[SRD-tangrowth-HomeSprite.png]]

*Vine Pokémon*
*While it remains still, it appears to be a large shrub. Unsuspecting prey get ensnared by its vines. In the summer its vines grow so large that you can't even see its eyes. Its arms regrow even if you cut them.*

**DexID**: 0465
**Name**: Tangrowth
**Type**: Grass
**Abilities**: [[SRD-Chlorophyll|Chlorophyll]] / [[SRD-Regenerator|Regenerator]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 6'6" / 2.0m
**Weight**: 563.0lbs / 256.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| From      | [[SRD-Tangela]] | Level  | Slow    |


## Learnset

![[Embedded Views.base#Learnsets]]