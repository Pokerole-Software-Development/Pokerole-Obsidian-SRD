---
Ability1: Blaze
Ability2: Flash Fire
AceMoves:
- Howl
- Double Kick
- Fire Pledge
AdvancedMoves:
- Swift
- Lava Plume
- Rollout
- Flamethrower
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-quilava-BookSprite.png
DexCategory: "Volcano Pok\xE9mon"
DexDescription: 'It intimidates foes with intense gusts of flames and superheated
  air. Beware: if a Quilava turns its back on you, it is planing on using a fire move.'
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Cyndaquil]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Typhlosion]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Typhlosion (Hisuian Form)]]'
  Region: Hisui
  Speed: Medium
ExpertMoves:
- Inferno
- Double-Edge
- Eruption
- Overheat
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.9
  Meters: 0.9
HiddenAbility: ''
HomeSprite: SRD-quilava-HomeSprite.png
Image: quilava.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 4
MaxVitality: 4
Name: Quilava
Number: 156
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Smokescreen
- Ember
Special: 2
StandardMoves:
- Quick Attack
- Flame Wheel
- Defense Curl
- Flame Charge
StarterMoves:
- Tackle
- Leer
Strength: 2
Type1: Fire
Type2: ''
Vitality: 2
Weight:
  Kilograms: 19.0
  Pounds: 41.0
---

#PokeroleSRD/Pokedex

# Quilava 

> [!grid]
> ![[SRD-quilava-BookSprite.png|wsmall]]
> ![[SRD-quilava-HomeSprite.png]]

*Volcano Pokémon*
*It intimidates foes with intense gusts of flames and superheated air. Beware: if a Quilava turns its back on you, it is planing on using a fire move.*

**DexID**: 0156
**Name**: Quilava
**Type**: Fire
**Abilities**: [[SRD-Blaze|Blaze]] / [[SRD-Flash Fire|Flash Fire]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'9" / 0.9m
**Weight**: 41.0lbs / 19.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon                           | Kind   | Speed   | Region   |
|:----------|:----------------------------------|:-------|:--------|:---------|
| From      | [[SRD-Cyndaquil]]                 | Level  | Medium  |          |
| To        | [[SRD-Typhlosion]]                | Level  | Medium  |          |
| To        | [[SRD-Typhlosion (Hisuian Form)]] | Level  | Medium  | Hisui    |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]