---
Ability1: Shed Skin
Ability2: Infiltrator
AceMoves:
- Giga Drain
- Scary Face
- Brutal Swing
AdvancedMoves:
- Glare
- Poison Fang
- Venom Drench
- Night Slash
- Gastro Acid
- Belch
- Haze
- Poison Jab
- Crunch
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-seviper-BookSprite.png
DexCategory: "Fang Snake Pok\xE9mon"
DexDescription: Their hate for the Zangoose has been boiling for so long it's now
  a basic instinct. They battle using their sword-edged poisonous tail, hiding in
  tall grass until an unwary prey gets close enough.
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Swords Dance
- Coil
- Wring Out
- Sludge Bomb
GenderType: ''
GoodStarter: false
Height:
  Feet: 8.8
  Meters: 2.7
HiddenAbility: ''
HomeSprite: SRD-seviper-HomeSprite.png
Image: seviper.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 6
MaxStrength: 6
MaxVitality: 4
Name: Seviper
Number: 336
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Bite
- Lick
Special: 3
StandardMoves:
- Feint
- Poison Tail
- Screech
- Venoshock
- Aqua Tail
- Iron Tail
StarterMoves:
- Wrap
- Swagger
Strength: 3
Type1: Poison
Type2: ''
Vitality: 2
Weight:
  Kilograms: 52.0
  Pounds: 115.0
---

#PokeroleSRD/Pokedex

# Seviper 

> [!grid]
> ![[SRD-seviper-BookSprite.png|wsmall]]
> ![[SRD-seviper-HomeSprite.png]]

*Fang Snake Pokémon*
*Their hate for the Zangoose has been boiling for so long it's now a basic instinct. They battle using their sword-edged poisonous tail, hiding in tall grass until an unwary prey gets close enough.*

**DexID**: 0336
**Name**: Seviper
**Type**: Poison
**Abilities**: [[SRD-Shed Skin|Shed Skin]] / [[SRD-Infiltrator|Infiltrator]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 8'8" / 2.7m
**Weight**: 115.0lbs / 52.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets]]