---
Ability1: Flash Fire
Ability2: White Smoke
AceMoves:
- Thunder Punch
- Night Slash
- Scorching Sands
AdvancedMoves:
- Stockpile
- Spit Up
- Swallow
- Bind
- Fire Punch
- Slash
- Fire Lash
- Hone Claws
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-heatmor-BookSprite.png
DexCategory: "Anteater Pok\xE9mon"
DexDescription: It draws in air through its tail, then transforms it into fire and
  uses it like a tongue. You can see them defending Durant's colonies from predators
  so they can be the only ones who can eat them.
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Amnesia
- Fire Spin
- Snatch
- Flamethrower
- Inferno
- Flare Blitz
GenderType: ''
GoodStarter: false
Height:
  Feet: 4.6
  Meters: 1.4
HiddenAbility: ''
HomeSprite: SRD-heatmor-HomeSprite.png
Image: heatmor.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 6
MaxStrength: 6
MaxVitality: 4
Name: Heatmor
Number: 631
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Fury Swipes
- Incinerate
Special: 3
StandardMoves:
- Odor Sleuth
- Bug Bite
- Flame Burst
- Sucker Punch
StarterMoves:
- Tackle
- Lick
Strength: 3
Type1: Fire
Type2: ''
Vitality: 2
Weight:
  Kilograms: 58.0
  Pounds: 127.0
---

#PokeroleSRD/Pokedex

# Heatmor 

> [!grid]
> ![[SRD-heatmor-BookSprite.png|wsmall]]
> ![[SRD-heatmor-HomeSprite.png]]

*Anteater Pokémon*
*It draws in air through its tail, then transforms it into fire and uses it like a tongue. You can see them defending Durant's colonies from predators so they can be the only ones who can eat them.*

**DexID**: 0631
**Name**: Heatmor
**Type**: Fire
**Abilities**: [[SRD-Flash Fire|Flash Fire]] / [[SRD-White Smoke|White Smoke]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 4'6" / 1.4m
**Weight**: 127.0lbs / 58.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets]]