---
Ability1: Levitate
Ability2: ''
AceMoves:
- Rage Powder
- Gastro Acid
- Attract
AdvancedMoves:
- Energy Ball
- Acid Spray
- Feint Attack
- Leaf Tornado
- Stockpile
- Spit Up
- Swallow
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-carnivine-BookSprite.png
DexCategory: "Bug Catcher Pok\xE9mon"
DexDescription: It binds itself to trees in marshes. It attracts prey with its sweet-smelling
  drool and gulps them down in one bite. It can take it a whole day to digest a single
  prey, but it won't need to eat for at least a week.
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Grass Knot
- Seed Bomb
- Crunch
- Wring Out
- Power Whip
GenderType: ''
GoodStarter: false
Height:
  Feet: 4.6
  Meters: 1.4
HiddenAbility: ''
HomeSprite: SRD-carnivine-HomeSprite.png
Image: carnivine.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 5
MaxSpecial: 5
MaxStrength: 6
MaxVitality: 5
Name: Carnivine
Number: 455
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Bite
- Vine Whip
- Sweet Scent
Special: 2
StandardMoves:
- Ingrain
- Stun Spore
- Sleep Powder
- Razor Leaf
- Cut
StarterMoves:
- Bind
- Growth
Strength: 3
Type1: Grass
Type2: ''
Vitality: 2
Weight:
  Kilograms: 27.0
  Pounds: 59.0
---

#PokeroleSRD/Pokedex

# Carnivine 

> [!grid]
> ![[SRD-carnivine-BookSprite.png|wsmall]]
> ![[SRD-carnivine-HomeSprite.png]]

*Bug Catcher Pokémon*
*It binds itself to trees in marshes. It attracts prey with its sweet-smelling drool and gulps them down in one bite. It can take it a whole day to digest a single prey, but it won't need to eat for at least a week.*

**DexID**: 0455
**Name**: Carnivine
**Type**: Grass
**Abilities**: [[SRD-Levitate|Levitate]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 4'6" / 1.4m
**Weight**: 59.0lbs / 27.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets]]