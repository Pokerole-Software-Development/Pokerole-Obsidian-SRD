---
Ability1: Merciless
Ability2: Regenerator
AceMoves:
- Sludge Bomb
- Iron Defense
- Spikes
AdvancedMoves:
- Baneful Bunker
- Infestation
- Wide Guard
- Recover
- Light Screen
- Toxic
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-toxapex-BookSprite.png
DexCategory: "Brutal Star Pok\xE9mon"
DexDescription: It crawls along the ocean floor, using its tentacles as a fortress.
  Its venom has the victim suffering for three days and nights, and even if it's healed,
  there are aftereffects, for it is a powerful toxin.
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Mareanie]]'
  Speed: Medium
ExpertMoves:
- Poison Jab
- Pain Split
- Safeguard
- Cross Poison
- Liquidation
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.3
  Meters: 0.7
HiddenAbility: ''
HomeSprite: SRD-toxapex-HomeSprite.png
Image: toxapex.png
Insight: 3
Legendary: false
MaxDexterity: 3
MaxInsight: 7
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 8
Name: Toxapex
Number: 748
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Bite
- Venoshock
Special: 2
StandardMoves:
- Pin Missile
- Toxic Spikes
- Spike Cannon
- Venom Drench
StarterMoves:
- Poison Sting
- Peck
Strength: 2
Type1: Poison
Type2: Water
Vitality: 4
Weight:
  Kilograms: 14.0
  Pounds: 32.0
---

#PokeroleSRD/Pokedex

# Toxapex 

> [!grid]
> ![[SRD-toxapex-BookSprite.png|wsmall]]
> ![[SRD-toxapex-HomeSprite.png]]

*Brutal Star Pokémon*
*It crawls along the ocean floor, using its tentacles as a fortress. Its venom has the victim suffering for three days and nights, and even if it's healed, there are aftereffects, for it is a powerful toxin.*

**DexID**: 0748
**Name**: Toxapex
**Type**: Poison / Water
**Abilities**: [[SRD-Merciless|Merciless]] / [[SRD-Regenerator|Regenerator]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⬤⬤⭘⭘⭘⭘  | 4/8  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |

**Height**: 2'3" / 0.7m
**Weight**: 32.0lbs / 14.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon          | Kind   | Speed   |
|:----------|:-----------------|:-------|:--------|
| From      | [[SRD-Mareanie]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]