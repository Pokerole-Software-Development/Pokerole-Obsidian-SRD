---
Ability1: Simple
Ability2: Moody
AceMoves:
- Bulldoze
- Rock Smash
- Stealth Rock
AdvancedMoves:
- Aqua Tail
- Swords Dance
- Rototiller
- Super Fang
- Hyper Fang
- Water Pulse
- Curse
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-bibarel-BookSprite.png
DexCategory: "Beaver Pok\xE9mon"
DexDescription: Bibarels build dams in streams with bark and mud. It is known as an
  industrious worker. Their constructions are very appreciated by people because a
  river dammed by Bibarel will never overflow.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Bidoof]]'
  Speed: Medium
ExpertMoves:
- Rest
- Surf
- Crunch
- Double-Edge
- Superpower
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.3
  Meters: 1.0
HiddenAbility: ''
HomeSprite: SRD-bibarel-HomeSprite.png
Image: bibarel.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 5
MaxVitality: 4
Name: Bibarel
Number: 400
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Defense Curl
- Rollout
Special: 2
StandardMoves:
- Water Gun
- Cut
- Headbutt
- Yawn
- Taunt
- Bite
- Amnesia
StarterMoves:
- Tackle
- Growl
Strength: 2
Type1: Normal
Type2: Water
Vitality: 2
Weight:
  Kilograms: 31.0
  Pounds: 69.0
---

#PokeroleSRD/Pokedex

# Bibarel 

> [!grid]
> ![[SRD-bibarel-BookSprite.png|wsmall]]
> ![[SRD-bibarel-HomeSprite.png]]

*Beaver Pokémon*
*Bibarels build dams in streams with bark and mud. It is known as an industrious worker. Their constructions are very appreciated by people because a river dammed by Bibarel will never overflow.*

**DexID**: 0400
**Name**: Bibarel
**Type**: Normal / Water
**Abilities**: [[SRD-Simple|Simple]] / [[SRD-Moody|Moody]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 3'3" / 1.0m
**Weight**: 69.0lbs / 31.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon        | Kind   | Speed   |
|:----------|:---------------|:-------|:--------|
| From      | [[SRD-Bidoof]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]