---
Ability1: Thick Fat
Ability2: Hydration
AceMoves:
- Lick
- Fake Out
- Entrainment
AdvancedMoves:
- Aqua Jet
- Brine
- Aqua Tail
- Dive
- Safeguard
- Ice Beam
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-seel-BookSprite.png
DexCategory: "Sea Lion Pok\xE9mon"
DexDescription: "A Pok\xE9mon that lives on icebergs. It swims in the sea using the\
  \ point on its head to break up the ice. It sleeps a lot during the day, being most\
  \ active at dawn when the temperature starts to cool."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Dewgong]]'
  Speed: Slow
ExpertMoves:
- Hail
GenderType: ''
GoodStarter: true
Height:
  Feet: 3.6
  Meters: 1.1
HiddenAbility: Ice Body
HomeSprite: SRD-seel-HomeSprite.png
Image: seel.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 5
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 4
Name: Seel
Number: 86
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Water Sport
- Water Gun
- Encore
Special: 2
StandardMoves:
- Icy Wind
- Take Down
- Ice Shard
- Rest
- Aqua Ring
- Aurora Beam
StarterMoves:
- Headbutt
- Growl
Strength: 2
Type1: Water
Type2: ''
Vitality: 2
Weight:
  Kilograms: 90.0
  Pounds: 198.0
---

#PokeroleSRD/Pokedex

# Seel 

> [!grid]
> ![[SRD-seel-BookSprite.png|wsmall]]
> ![[SRD-seel-HomeSprite.png]]

*Sea Lion Pokémon*
*A Pokémon that lives on icebergs. It swims in the sea using the point on its head to break up the ice. It sleeps a lot during the day, being most active at dawn when the temperature starts to cool.*

**DexID**: 0086
**Name**: Seel
**Type**: Water
**Abilities**: [[SRD-Thick Fat|Thick Fat]] / [[SRD-Hydration|Hydration]] ([[SRD-Ice Body|Ice Body]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 3'6" / 1.1m
**Weight**: 198.0lbs / 90.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Dewgong]] | Level  | Slow    |


## Learnset

![[Embedded Views.base#Learnsets]]