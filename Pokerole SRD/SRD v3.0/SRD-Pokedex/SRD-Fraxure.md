---
Ability1: Rivalry
Ability2: Unnerve
AceMoves:
- Night Slash
- First Impression
- Counter
AdvancedMoves:
- Scary Face
- Dragon Rage
- Crunch
- Dragon Dance
- Dual Chop
- Laser Focus
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-fraxure-BookSprite.png
DexCategory: "Axe Jaw Pok\xE9mon"
DexDescription: Their tusks can shatter rocks. Territory battles between Fraxure can
  be intensely violent. The tusks don't grow back anymore; if you find a Fraxure with
  both tusks whole, it means it's one of the strongest.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Axew]]'
  Speed: Slow
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Haxorus]]'
  Speed: Slow
ExpertMoves:
- Dragon Pulse
- Swords Dance
- Outrage
- Guillotine
- Giga Impact
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.3
  Meters: 1.0
HiddenAbility: ''
HomeSprite: SRD-fraxure-HomeSprite.png
Image: fraxure.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 3
MaxStrength: 6
MaxVitality: 5
Name: Fraxure
Number: 611
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Bite
- False Swipe
Special: 1
StandardMoves:
- Assurance
- Taunt
- Slash
- Dragon Claw
StarterMoves:
- Scratch
- Leer
Strength: 3
Type1: Dragon
Type2: ''
Vitality: 2
Weight:
  Kilograms: 36.0
  Pounds: 79.0
---

#PokeroleSRD/Pokedex

# Fraxure 

> [!grid]
> ![[SRD-fraxure-BookSprite.png|wsmall]]
> ![[SRD-fraxure-HomeSprite.png]]

*Axe Jaw Pokémon*
*Their tusks can shatter rocks. Territory battles between Fraxure can be intensely violent. The tusks don't grow back anymore; if you find a Fraxure with both tusks whole, it means it's one of the strongest.*

**DexID**: 0611
**Name**: Fraxure
**Type**: Dragon
**Abilities**: [[SRD-Rivalry|Rivalry]] / [[SRD-Unnerve|Unnerve]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 3'3" / 1.0m
**Weight**: 79.0lbs / 36.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| From      | [[SRD-Axew]]    | Level  | Slow    |
| To        | [[SRD-Haxorus]] | Level  | Slow    |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]