---
Ability1: Pickup
Ability2: Quick Feet
AceMoves:
- Parting Shot
- Knock Off
- Quick Guard
AdvancedMoves:
- Headbutt
- Fury Swipes
- Switcheroo
- Flail
- Hone Claws
- Rest
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-linoone-galarian-form-BookSprite.png
DexCategory: "Rushing Pok\xE9mon"
DexDescription: "It uses its long tongue to taunt its opponents. Once the foe is enraged,\
  \ this Pok\xE9mon hurls itself at it, tackling them forcefully. They are bad-mannered\
  \ and enjoy thrashing things about."
Dexterity: 3
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Zigzagoon (Galarian Form)]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Obstagoon]]'
  Speed: Medium
ExpertMoves:
- Scary Face
- Night Slash
- Double-Edge
- Counter
GenderType: ''
GoodStarter: false
Height:
  Feet: 1.6
  Meters: 0.5
HiddenAbility: ''
HomeSprite: SRD-linoone-galarian-form-HomeSprite.png
Image: linoone-galarian-form.png
Insight: 2
Legendary: false
MaxDexterity: 6
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 5
MaxVitality: 4
Name: Linoone (Galarian Form)
Number: 264
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Sand Attack
- Lick
Special: 2
StandardMoves:
- Take Down
- Taunt
- Baby-Doll Eyes
- Snarl
- Pin Missile
- Odor Sleuth
StarterMoves:
- Tackle
- Leer
Strength: 2
Type1: Dark
Type2: Normal
Vitality: 2
Weight:
  Kilograms: 32.0
  Pounds: 71.0
---

#PokeroleSRD/Pokedex

# Linoone (Galarian Form) 

> [!grid]
> ![[SRD-linoone-galarian-form-BookSprite.png|wsmall]]
> ![[SRD-linoone-galarian-form-HomeSprite.png]]

*Rushing Pokémon*
*It uses its long tongue to taunt its opponents. Once the foe is enraged, this Pokémon hurls itself at it, tackling them forcefully. They are bad-mannered and enjoy thrashing things about.*

**DexID**: 0264G
**Name**: Linoone (Galarian Form)
**Type**: Dark / Normal
**Abilities**: [[SRD-Pickup|Pickup]] / [[SRD-Quick Feet|Quick Feet]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⬤⭘⭘⭘ | 3/6 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'6" / 0.5m
**Weight**: 71.0lbs / 32.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon                           | Kind   | Speed   |
|:----------|:----------------------------------|:-------|:--------|
| From      | [[SRD-Zigzagoon (Galarian Form)]] | Level  | Medium  |
| To        | [[SRD-Obstagoon]]                 | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]