---
Ability1: Natural Cure
Ability2: Harvest
AceMoves:
- Venom Drench
- Grudge
- Trick Room
AdvancedMoves:
- Feint Attack
- Horn Leech
- Curse
- Ingrain
- Imprison
- Disable
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-trevenant-BookSprite.png
DexCategory: "Elder Tree Pok\xE9mon"
DexDescription: "Using its roots as a nervous system, it controls the trees in the\
  \ forest. It's kind to the Pok\xE9mon that reside inside its body, but it is ruthless\
  \ to anyone that harms its forest, turning them into haunted trees forever."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Trade
  Pokemon: '[[SRD-Phantump]]'
ExpertMoves:
- Shadow Claw
- Brutal Swing
- Phantom Force
- Wood Hammer
- Destiny Bond
- Forest's Curse
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.0
  Meters: 1.5
HiddenAbility: ''
HomeSprite: SRD-trevenant-HomeSprite.png
Image: trevenant.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 5
MaxSpecial: 4
MaxStrength: 6
MaxVitality: 5
Name: Trevenant
Number: 709
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Branch Poke
- Leech Seed
Special: 2
StandardMoves:
- Confuse Ray
- Will-O-Wisp
- Hex
- Growth
StarterMoves:
- Astonish
- Tackle
Strength: 3
Type1: Ghost
Type2: Grass
Vitality: 2
Weight:
  Kilograms: 71.0
  Pounds: 156.0
---

#PokeroleSRD/Pokedex

# Trevenant 

> [!grid]
> ![[SRD-trevenant-BookSprite.png|wsmall]]
> ![[SRD-trevenant-HomeSprite.png]]

*Elder Tree Pokémon*
*Using its roots as a nervous system, it controls the trees in the forest. It's kind to the Pokémon that reside inside its body, but it is ruthless to anyone that harms its forest, turning them into haunted trees forever.*

**DexID**: 0709
**Name**: Trevenant
**Type**: Ghost / Grass
**Abilities**: [[SRD-Natural Cure|Natural Cure]] / [[SRD-Harvest|Harvest]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 5'0" / 1.5m
**Weight**: 156.0lbs / 71.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon          | Kind   |
|:----------|:-----------------|:-------|
| From      | [[SRD-Phantump]] | Trade  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]