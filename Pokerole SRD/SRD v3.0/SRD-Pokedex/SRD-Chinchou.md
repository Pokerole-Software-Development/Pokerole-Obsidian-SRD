---
Ability1: Illuminate
Ability2: Volt Absorb
AceMoves:
- Agility
- Soak
- Psybeam
AdvancedMoves:
- Signal Beam
- Bubble Beam
- Aqua Ring
- Discharge
- Ion Deluge
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-chinchou-BookSprite.png
DexCategory: "Angler Pok\xE9mon"
DexDescription: "In the dark ocean floor, its only means of communication is to constantly\
  \ flash its lights. It is a clumsy but friendly Pok\xE9mon. Its antennae can be\
  \ used to power up small electric appliances."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Lanturn]]'
  Speed: Medium
ExpertMoves:
- Hydro Pump
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.6
  Meters: 0.5
HiddenAbility: Water Absorb
HomeSprite: SRD-chinchou-HomeSprite.png
Image: chinchou.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 3
MaxVitality: 3
Name: Chinchou
Number: 170
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Thunder Wave
- Flail
Special: 2
StandardMoves:
- Bubble
- Confuse Ray
- Spark
- Take Down
- Electro Ball
- Charge
StarterMoves:
- Water Gun
- Supersonic
Strength: 1
Type1: Water
Type2: Electric
Vitality: 1
Weight:
  Kilograms: 12.0
  Pounds: 26.0
---

#PokeroleSRD/Pokedex

# Chinchou 

> [!grid]
> ![[SRD-chinchou-BookSprite.png|wsmall]]
> ![[SRD-chinchou-HomeSprite.png]]

*Angler Pokémon*
*In the dark ocean floor, its only means of communication is to constantly flash its lights. It is a clumsy but friendly Pokémon. Its antennae can be used to power up small electric appliances.*

**DexID**: 0170
**Name**: Chinchou
**Type**: Water / Electric
**Abilities**: [[SRD-Illuminate|Illuminate]] / [[SRD-Volt Absorb|Volt Absorb]] ([[SRD-Water Absorb|Water Absorb]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⭘⭘  | 1/3  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'6" / 0.5m
**Weight**: 26.0lbs / 12.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Lanturn]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]