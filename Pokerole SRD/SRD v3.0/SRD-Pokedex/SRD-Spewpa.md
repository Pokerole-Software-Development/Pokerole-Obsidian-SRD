---
Ability1: Shed Skin
Ability2: ''
AceMoves: []
AdvancedMoves:
- Electroweb
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-spewpa-BookSprite.png
DexCategory: "Scatterdust Pok\xE9mon"
DexDescription: "It remains hidden inside old logs. When predators attack, it quickly\
  \ bristles the fur covering its body to scare them. Bird Pok\xE9mon have a hard\
  \ time trying to eat it with all the dust it releases as protection."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Scatterbug]]'
  Speed: Fast
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Vivillon]]'
  Speed: Fast
ExpertMoves: []
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.0
  Meters: 0.3
HiddenAbility: Friend Guard
HomeSprite: SRD-spewpa-HomeSprite.png
Image: spewpa.png
Insight: 1
Legendary: false
MaxDexterity: 3
MaxInsight: 3
MaxSpecial: 3
MaxStrength: 3
MaxVitality: 4
Name: Spewpa
Number: 665
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Protect
Special: 1
StandardMoves:
- Iron Defense
StarterMoves:
- Harden
Strength: 1
Type1: Bug
Type2: ''
Vitality: 2
Weight:
  Kilograms: 8.0
  Pounds: 18.0
---

#PokeroleSRD/Pokedex

# Spewpa 

> [!grid]
> ![[SRD-spewpa-BookSprite.png|wsmall]]
> ![[SRD-spewpa-HomeSprite.png]]

*Scatterdust Pokémon*
*It remains hidden inside old logs. When predators attack, it quickly bristles the fur covering its body to scare them. Bird Pokémon have a hard time trying to eat it with all the dust it releases as protection.*

**DexID**: 0665
**Name**: Spewpa
**Type**: Bug
**Abilities**: [[SRD-Shed Skin|Shed Skin]] ([[SRD-Friend Guard|Friend Guard]])
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⭘⭘   | 1/3   |

**Height**: 1'0" / 0.3m
**Weight**: 18.0lbs / 8.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon            | Kind   | Speed   |
|:----------|:-------------------|:-------|:--------|
| From      | [[SRD-Scatterbug]] | Level  | Fast    |
| To        | [[SRD-Vivillon]]   | Level  | Fast    |


## Learnset

![[Embedded Views.base#Learnsets]]