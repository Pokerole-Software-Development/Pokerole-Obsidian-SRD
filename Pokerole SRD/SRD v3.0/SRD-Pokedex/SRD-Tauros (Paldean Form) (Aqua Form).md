---
Ability1: Intimidate
Ability2: Anger Point
AceMoves:
- Liquidation
- Drill Run
- Outrage
AdvancedMoves:
- Payback
- Raging Bull
- Rest
- Zen Headbutt
- Assurance
- Swagger
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-tauros-paldean-form-aqua-form-BookSprite.png
DexCategory: "Wild Bull Pok\xE9mon"
DexDescription: This is a male species. These Tauros are specially bred in Paldea
  for their ability to travel in water. Their body is thick with a fat layer under
  the skin that helps them float. They shoot powerful water jets from their horns.
Dexterity: 3
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Thrash
- Wave Crash
- Lash Out
- Close Combat
GenderType: M
GoodStarter: false
Height:
  Feet: 4.6
  Meters: 1.4
HiddenAbility: Sheer Force
HomeSprite: SRD-tauros-paldean-form-aqua-form-HomeSprite.png
Image: tauros-paldean-form-aqua-form.png
Insight: 2
Legendary: false
MaxDexterity: 6
MaxInsight: 5
MaxSpecial: 2
MaxStrength: 5
MaxVitality: 7
Name: Tauros (Paldean Form) (Aqua Form)
Number: 128
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Rage
- Double Kick
Special: 1
StandardMoves:
- Work Up
- Scary Face
- Aqua Jet
- Headbutt
StarterMoves:
- Tackle
- Tail Whip
Strength: 2
Type1: Fighting
Type2: Water
Vitality: 3
Weight:
  Kilograms: 220.0
  Pounds: 485.0
---

#PokeroleSRD/Pokedex

# Tauros (Paldean Form) (Aqua Form) 

> [!grid]
> ![[SRD-tauros-paldean-form-aqua-form-BookSprite.png|wsmall]]
> ![[SRD-tauros-paldean-form-aqua-form-HomeSprite.png]]

*Wild Bull Pokémon*
*This is a male species. These Tauros are specially bred in Paldea for their ability to travel in water. Their body is thick with a fat layer under the skin that helps them float. They shoot powerful water jets from their horns.*

**DexID**: 0128PA
**Name**: Tauros (Paldean Form) (Aqua Form)
**Type**: Fighting / Water
**Abilities**: [[SRD-Intimidate|Intimidate]] / [[SRD-Anger Point|Anger Point]] ([[SRD-Sheer Force|Sheer Force]])
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⬤⭘⭘⭘ | 3/6 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Special   | ⬤⭘   | 1/2   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 4'6" / 1.4m
**Weight**: 485.0lbs / 220.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert


## Learnset

![[Embedded Views.base#Learnsets v3.0]]