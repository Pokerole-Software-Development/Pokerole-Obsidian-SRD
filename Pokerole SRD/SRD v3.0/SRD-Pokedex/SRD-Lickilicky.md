---
Ability1: Oblivious
Ability2: Cloud Nine
AceMoves:
- Steel Roller
- Body Press
- Aqua Tail
AdvancedMoves:
- Screech
- Slam
- Me First
- Chip Away
- Ice Ball
- Rest
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-lickilicky-BookSprite.png
DexCategory: "Licking Pok\xE9mon"
DexDescription: It uses its tongue as a stretchable arm. It will wrap prey with it
  and then proceed to eat it. Its saliva causes numbness. Try not to touch its tongue
  as it might try to eat you by reflex.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Lickitung]]'
  Speed: Slow
ExpertMoves:
- Refresh
- Wring Out
- Power Whip
- Belly Drum
- Giga Impact
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.6
  Meters: 1.7
HiddenAbility: ''
HomeSprite: SRD-lickilicky-HomeSprite.png
Image: lickilicky.png
Insight: 3
Legendary: false
MaxDexterity: 4
MaxInsight: 6
MaxSpecial: 5
MaxStrength: 5
MaxVitality: 6
Name: Lickilicky
Number: 463
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Rollout
- Supersonic
Special: 2
StandardMoves:
- Wrap
- Disable
- Stomp
- Knock Off
- Bulldoze
- Zen Headbutt
StarterMoves:
- Lick
- Defense Curl
Strength: 2
Type1: Normal
Type2: ''
Vitality: 3
Weight:
  Kilograms: 280.0
  Pounds: 616.0
---

#PokeroleSRD/Pokedex

# Lickilicky 

> [!grid]
> ![[SRD-lickilicky-BookSprite.png|wsmall]]
> ![[SRD-lickilicky-HomeSprite.png]]

*Licking Pokémon*
*It uses its tongue as a stretchable arm. It will wrap prey with it and then proceed to eat it. Its saliva causes numbness. Try not to touch its tongue as it might try to eat you by reflex.*

**DexID**: 0463
**Name**: Lickilicky
**Type**: Normal
**Abilities**: [[SRD-Oblivious|Oblivious]] / [[SRD-Cloud Nine|Cloud Nine]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 5'6" / 1.7m
**Weight**: 616.0lbs / 280.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon           | Kind   | Speed   |
|:----------|:------------------|:-------|:--------|
| From      | [[SRD-Lickitung]] | Level  | Slow    |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]