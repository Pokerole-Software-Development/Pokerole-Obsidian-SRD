---
Ability1: Vital Spirit
Ability2: Pickup
AceMoves:
- Lick
- Endure
- Yawn
AdvancedMoves:
- Retaliate
- Reversal
- Play Rough
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-lillipup-BookSprite.png
DexCategory: "Puppy Pok\xE9mon"
DexDescription: "Good with children and old people, this gentle Pok\xE9mon is a favorite\
  \ to keep as pet. It is very brave and smart. It will protect its trainer against\
  \ any threat. It uses the fur on its face to feel its surroundings."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Herdier]]'
  Speed: Medium
ExpertMoves:
- Giga Impact
- Last Resort
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.3
  Meters: 0.4
HiddenAbility: Scrappy
HomeSprite: SRD-lillipup-HomeSprite.png
Image: lillipup.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 3
MaxStrength: 4
MaxVitality: 4
Name: Lillipup
Number: 506
ProMoves: []
RecommendedRank: Starter
RookieMoves:
- Odor Sleuth
- Bite
Special: 1
StandardMoves:
- Baby-Doll Eyes
- Helping Hand
- Take Down
- Work Up
- Crunch
- Roar
StarterMoves:
- Tackle
- Leer
Strength: 2
Type1: Normal
Type2: ''
Vitality: 2
Weight:
  Kilograms: 4.0
  Pounds: 9.0
---

#PokeroleSRD/Pokedex

# Lillipup 

> [!grid]
> ![[SRD-lillipup-BookSprite.png|wsmall]]
> ![[SRD-lillipup-HomeSprite.png]]

*Puppy Pokémon*
*Good with children and old people, this gentle Pokémon is a favorite to keep as pet. It is very brave and smart. It will protect its trainer against any threat. It uses the fur on its face to feel its surroundings.*

**DexID**: 0506
**Name**: Lillipup
**Type**: Normal
**Abilities**: [[SRD-Vital Spirit|Vital Spirit]] / [[SRD-Pickup|Pickup]] ([[SRD-Scrappy|Scrappy]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'3" / 0.4m
**Weight**: 9.0lbs / 4.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Starter

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Herdier]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]