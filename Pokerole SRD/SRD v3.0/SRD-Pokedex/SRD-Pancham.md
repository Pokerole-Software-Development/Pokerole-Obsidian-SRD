---
Ability1: Iron Fist
Ability2: Scrappy
AceMoves:
- Fire Punch
- Ice Punch
- Thunder Punch
AdvancedMoves:
- Vital Throw
- Crunch
- Body Slam
- Parting Shot
- Entrainment
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-pancham-BookSprite.png
DexCategory: "Playful Pok\xE9mon"
DexDescription: It lives in bamboo forests. It is very energetic and playful. It wishes
  to be taken seriously but has a hard time due to its cute appearance. For this reason
  it may start hanging out with the wrong crowd.
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Pangoro]]'
  Speed: Medium
ExpertMoves:
- Sky Uppercut
GenderType: ''
GoodStarter: true
Height:
  Feet: 2.0
  Meters: 0.6
HiddenAbility: ''
HomeSprite: SRD-pancham-HomeSprite.png
Image: pancham.png
Insight: 2
Legendary: false
MaxDexterity: 3
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 5
MaxVitality: 4
Name: Pancham
Number: 674
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Arm Thrust
- Taunt
Special: 2
StandardMoves:
- Circle Throw
- Comet Punch
- Low Sweep
- Work Up
- Slash
- Karate Chop
StarterMoves:
- Tackle
- Leer
Strength: 2
Type1: Fighting
Type2: ''
Vitality: 2
Weight:
  Kilograms: 8.0
  Pounds: 17.0
---

#PokeroleSRD/Pokedex

# Pancham 

> [!grid]
> ![[SRD-pancham-BookSprite.png|wsmall]]
> ![[SRD-pancham-HomeSprite.png]]

*Playful Pokémon*
*It lives in bamboo forests. It is very energetic and playful. It wishes to be taken seriously but has a hard time due to its cute appearance. For this reason it may start hanging out with the wrong crowd.*

**DexID**: 0674
**Name**: Pancham
**Type**: Fighting
**Abilities**: [[SRD-Iron Fist|Iron Fist]] / [[SRD-Scrappy|Scrappy]]
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'0" / 0.6m
**Weight**: 17.0lbs / 8.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Pangoro]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]