---
Ability1: Pressure
Ability2: Telepathy
AceMoves: []
AdvancedMoves: []
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-palkia-BookSprite.png
DexCategory: "Pok\xE9dex has no data."
DexDescription: "An old scroll describes a being who is capable of bending and twisting\
  \ space in impossible ways by a powerful Pok\xE9mon god."
Dexterity: 6
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Form
  Pokemon: '[[SRD-Palkia (Origin Form)]]'
ExpertMoves: []
GenderType: N
GoodStarter: false
Height:
  Feet: 13.8
  Meters: 4.2
HiddenAbility: ''
HomeSprite: SRD-palkia-HomeSprite.png
Image: palkia.png
Insight: 7
Legendary: true
MaxDexterity: 6
MaxInsight: 7
MaxSpecial: 8
MaxStrength: 7
MaxVitality: 6
Name: Palkia
Number: 484
ProMoves: []
RecommendedRank: Champion
RookieMoves: []
Special: 8
StandardMoves: []
StarterMoves: []
Strength: 7
Type1: Water
Type2: Dragon
Vitality: 6
Weight:
  Kilograms: 336.0
  Pounds: 740.0
---

#PokeroleSRD/Pokedex

# Palkia 

> [!grid]
> ![[SRD-palkia-BookSprite.png|wsmall]]
> ![[SRD-palkia-HomeSprite.png]]

*Pokédex has no data.*
*An old scroll describes a being who is capable of bending and twisting space in impossible ways by a powerful Pokémon god.*

**DexID**: 0484
**Name**: Palkia
**Type**: Water / Dragon
**Abilities**: [[SRD-Pressure|Pressure]] / [[SRD-Telepathy|Telepathy]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⬤⬤⬤⬤  | 7/7  |
| Dexterity | ⬤⬤⬤⬤⬤⬤ | 6/6 |
| Vitality  | ⬤⬤⬤⬤⬤⬤  | 6/6  |
| Special   | ⬤⬤⬤⬤⬤⬤⬤⬤   | 8/8   |
| Insight   | ⬤⬤⬤⬤⬤⬤⬤   | 7/7   |

**Height**: 13'8" / 4.2m
**Weight**: 740.0lbs / 336.0kg
**Good Starter**:: No
**Recommended Rank**:: Champion

| Evolves   | Pokemon                      | Kind   |
|:----------|:-----------------------------|:-------|
| To        | [[SRD-Palkia (Origin Form)]] | Form   |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]