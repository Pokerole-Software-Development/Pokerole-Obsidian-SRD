---
Ability1: Cloud Nine
Ability2: Damp
AceMoves:
- Clear Smog
- Confuse Ray
- Future Sight
AdvancedMoves:
- Amnesia
- Psych Up
- Hydro Pump
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-psyduck-BookSprite.png
DexCategory: "Duck Pok\xE9mon"
DexDescription: It lives near lakes and ponds, but it's not very good at swimming.
  It is always tormented by headaches that worsen when it uses psychic powers. Psyduck
  seem unaware of their own power.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Golduck]]'
  Speed: Medium
ExpertMoves:
- Wonder Room
GenderType: ''
GoodStarter: true
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: Swift Swim
HomeSprite: SRD-psyduck-HomeSprite.png
Image: psyduck.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 4
Name: Psyduck
Number: 54
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Water Gun
- Disable
- Confusion
Special: 2
StandardMoves:
- Water Pulse
- Fury Swipes
- Screech
- Zen Headbutt
- Aqua Tail
- Soak
StarterMoves:
- Water Sport
- Scratch
- Tail Whip
Strength: 2
Type1: Water
Type2: ''
Vitality: 2
Weight:
  Kilograms: 20.0
  Pounds: 44.0
---

#PokeroleSRD/Pokedex

# Psyduck 

> [!grid]
> ![[SRD-psyduck-BookSprite.png|wsmall]]
> ![[SRD-psyduck-HomeSprite.png]]

*Duck Pokémon*
*It lives near lakes and ponds, but it's not very good at swimming. It is always tormented by headaches that worsen when it uses psychic powers. Psyduck seem unaware of their own power.*

**DexID**: 0054
**Name**: Psyduck
**Type**: Water
**Abilities**: [[SRD-Cloud Nine|Cloud Nine]] / [[SRD-Damp|Damp]] ([[SRD-Swift Swim|Swift Swim]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'6" / 0.8m
**Weight**: 44.0lbs / 20.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Golduck]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]