---
Ability1: Neuroforce
Ability2: ''
AceMoves:
- Moonlight
- Morning Sun
- Charge Beam
- Gravity
- Metal Claw
- Mirror Shot
- Confusion
- Stealth Rock
- Slash
- Psycho Cut
- Stored Power
- Rock Blast
- Iron Defense
- Power Gem
- Night Slash
- Shadow Claw
- Wring Out
- Autotomize
- Light Screen
- Reflect
- Outrage
- Cosmic Power
- Prismatic Laser
- Photon Geyser
- Heat Wave
- Solar Beam
- Moongeist Beam
- Sunsteel Strike
AdvancedMoves: []
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-necrozma-ultra-burst-form-BookSprite.png
DexCategory: Aether Foundation S.O.S Message (Recovered)
DexDescription: "Alert to all international authorities! There\u2019s been a (White\
  \ Noise) serious breach of security! This is an emergency! Close the port-(White\
  \ Noise). NOW! (White Noise)."
Dexterity: 7
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Special
  Pokemon: '[[SRD-Necrozma]]'
ExpertMoves: []
GenderType: N
GoodStarter: false
Height:
  Feet: 24.6
  Meters: 7.5
HiddenAbility: ''
HomeSprite: SRD-necrozma-ultra-burst-form-HomeSprite.png
Image: necrozma-ultra-burst-form.png
Insight: 6
Legendary: true
MaxDexterity: 7
MaxInsight: 6
MaxSpecial: 8
MaxStrength: 8
MaxVitality: 6
Name: Necrozma (Ultra Burst Form)
Number: 800
ProMoves: []
RecommendedRank: Master
RookieMoves: []
Special: 8
StandardMoves: []
StarterMoves: []
Strength: 8
Type1: Psychic
Type2: Dragon
Vitality: 6
Weight:
  Kilograms: 230
  Pounds: 507
---

#PokeroleSRD/Pokedex

# Necrozma (Ultra Burst Form) 

> [!grid]
> ![[SRD-necrozma-ultra-burst-form-BookSprite.png|wsmall]]
> ![[SRD-necrozma-ultra-burst-form-HomeSprite.png]]

*Aether Foundation S.O.S Message (Recovered)*
*Alert to all international authorities! There’s been a (White Noise) serious breach of security! This is an emergency! Close the port-(White Noise). NOW! (White Noise).*

**DexID**: 0800F3
**Name**: Necrozma (Ultra Burst Form)
**Type**: Psychic / Dragon
**Abilities**: [[SRD-Neuroforce|Neuroforce]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⬤⬤⬤⬤⬤  | 8/8  |
| Dexterity | ⬤⬤⬤⬤⬤⬤⬤ | 7/7 |
| Vitality  | ⬤⬤⬤⬤⬤⬤  | 6/6  |
| Special   | ⬤⬤⬤⬤⬤⬤⬤⬤   | 8/8   |
| Insight   | ⬤⬤⬤⬤⬤⬤   | 6/6   |

**Height**: 24'6" / 7.5m
**Weight**: 507lbs / 230kg
**Good Starter**:: No
**Recommended Rank**:: Master

| Evolves   | Pokemon          | Kind    |
|:----------|:-----------------|:--------|
| From      | [[SRD-Necrozma]] | Special |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]