---
Ability1: Overgrow
Ability2: Contrary
AceMoves:
- Grass Pledge
- Synthesis
- Twister
AdvancedMoves:
- Leech Seed
- Mega Drain
- Slam
- Leaf Blade
- Coil
- Giga Drain
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-servine-BookSprite.png
DexCategory: "Grass Snake Pok\xE9mon"
DexDescription: It moves by slithering along the ground. Its swift movements befuddle
  its foes before it attacks with a vine whip. It always keeps itself clean and it
  demands care from its trainer, otherwise it will misbehave.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Snivy]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Serperior]]'
  Speed: Medium
ExpertMoves:
- Wring Out
- Gastro Acid
- Leaf Storm
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: ''
HomeSprite: SRD-servine-HomeSprite.png
Image: servine.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 5
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 5
Name: Servine
Number: 496
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Vine Whip
- Wrap
Special: 2
StandardMoves:
- Growth
- Leaf Tornado
StarterMoves:
- Tackle
- Leer
Strength: 2
Type1: Grass
Type2: ''
Vitality: 2
Weight:
  Kilograms: 16.0
  Pounds: 35.0
---

#PokeroleSRD/Pokedex

# Servine 

> [!grid]
> ![[SRD-servine-BookSprite.png|wsmall]]
> ![[SRD-servine-HomeSprite.png]]

*Grass Snake Pokémon*
*It moves by slithering along the ground. Its swift movements befuddle its foes before it attacks with a vine whip. It always keeps itself clean and it demands care from its trainer, otherwise it will misbehave.*

**DexID**: 0496
**Name**: Servine
**Type**: Grass
**Abilities**: [[SRD-Overgrow|Overgrow]] / [[SRD-Contrary|Contrary]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 2'6" / 0.8m
**Weight**: 35.0lbs / 16.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon           | Kind   | Speed   |
|:----------|:------------------|:-------|:--------|
| From      | [[SRD-Snivy]]     | Level  | Medium  |
| To        | [[SRD-Serperior]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]