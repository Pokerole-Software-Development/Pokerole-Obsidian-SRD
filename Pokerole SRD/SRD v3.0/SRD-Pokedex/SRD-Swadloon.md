---
Ability1: Leaf Guard
Ability2: Overcoat
AceMoves:
- Camouflage
- Seed Bomb
- Iron Defense
AdvancedMoves:
- Bug Buzz
- Protect
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-swadloon-BookSprite.png
DexCategory: "Leaf-Wrapped Pok\xE9mon"
DexDescription: Preferring dark and damp places, it spends the entire day inside of
  rotting logs. It protects itself from the cold by wrapping up in leaves. It's kind
  of moody and it likes to remain undisturbed.
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Sewaddle]]'
  Speed: Fast
- Evolves: To
  Kind: Stat
  Pokemon: '[[SRD-Leavanny]]'
  Stat: Happiness
  Value: 4
ExpertMoves: []
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.7
  Meters: 0.5
HiddenAbility: ''
HomeSprite: SRD-swadloon-HomeSprite.png
Image: swadloon.png
Insight: 2
Legendary: false
MaxDexterity: 3
MaxInsight: 5
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 5
Name: Swadloon
Number: 541
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Bug Bite
- Razor Leaf
Special: 2
StandardMoves:
- Struggle Bug
- Endure
- Grass Whistle
StarterMoves:
- Tackle
- String Shot
Strength: 2
Type1: Bug
Type2: Grass
Vitality: 2
Weight:
  Kilograms: 7.0
  Pounds: 16.0
---

#PokeroleSRD/Pokedex

# Swadloon 

> [!grid]
> ![[SRD-swadloon-BookSprite.png|wsmall]]
> ![[SRD-swadloon-HomeSprite.png]]

*Leaf-Wrapped Pokémon*
*Preferring dark and damp places, it spends the entire day inside of rotting logs. It protects itself from the cold by wrapping up in leaves. It's kind of moody and it likes to remain undisturbed.*

**DexID**: 0541
**Name**: Swadloon
**Type**: Bug / Grass
**Abilities**: [[SRD-Leaf Guard|Leaf Guard]] / [[SRD-Overcoat|Overcoat]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 1'7" / 0.5m
**Weight**: 16.0lbs / 7.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon          | Kind   | Speed   | Stat      | Value   |
|:----------|:-----------------|:-------|:--------|:----------|:--------|
| From      | [[SRD-Sewaddle]] | Level  | Fast    |           |         |
| To        | [[SRD-Leavanny]] | Stat   |         | Happiness | 4.0     |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]