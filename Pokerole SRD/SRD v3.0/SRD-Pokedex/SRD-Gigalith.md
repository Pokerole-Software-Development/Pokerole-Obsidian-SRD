---
Ability1: Sturdy
Ability2: Sand Stream
AceMoves:
- Body Press
- Superpower
- Wide Guard
AdvancedMoves:
- Iron Defense
- Smack Down
- Power Gem
- Rock Slide
- Stealth Rock
- Sandstorm
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-gigalith-BookSprite.png
DexCategory: "Compressed Pok\xE9mon"
DexDescription: "It is a serious Pok\xE9mon that doesn't interact with others too\
  \ much. It uses the sharp crystals on its body to recharge using the sun's energy.\
  \ If angered, it can bury its foe under giant rock slides or explode at will."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Trade
  Pokemon: '[[SRD-Boldore]]'
ExpertMoves:
- Gravity
- Rock Smash
- Stomping Tantrum
- Heavy Slam
- Stone Edge
- Explosion
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.6
  Meters: 1.7
HiddenAbility: Sand Force
HomeSprite: SRD-gigalith-HomeSprite.png
Image: gigalith.png
Insight: 2
Legendary: false
MaxDexterity: 3
MaxInsight: 5
MaxSpecial: 4
MaxStrength: 7
MaxVitality: 7
Name: Gigalith
Number: 526
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Sand Attack
- Headbutt
Special: 2
StandardMoves:
- Rock Blast
- Mud Slap
StarterMoves:
- Tackle
- Harden
Strength: 3
Type1: Rock
Type2: ''
Vitality: 3
Weight:
  Kilograms: 520.0
  Pounds: 1144.0
---

#PokeroleSRD/Pokedex

# Gigalith 

> [!grid]
> ![[SRD-gigalith-BookSprite.png|wsmall]]
> ![[SRD-gigalith-HomeSprite.png]]

*Compressed Pokémon*
*It is a serious Pokémon that doesn't interact with others too much. It uses the sharp crystals on its body to recharge using the sun's energy. If angered, it can bury its foe under giant rock slides or explode at will.*

**DexID**: 0526
**Name**: Gigalith
**Type**: Rock
**Abilities**: [[SRD-Sturdy|Sturdy]] / [[SRD-Sand Stream|Sand Stream]] ([[SRD-Sand Force|Sand Force]])
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 5'6" / 1.7m
**Weight**: 1144.0lbs / 520.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon         | Kind   |
|:----------|:----------------|:-------|
| From      | [[SRD-Boldore]] | Trade  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]