---
Ability1: Battle Armor
Ability2: Defiant
AceMoves:
- Agility
- Swords Dance
- Substitute
AdvancedMoves:
- Endure
- Reversal
- First Impression
- No Retreat
- Beat Up
- Iron Head
- Iron Defense
- Poison Jab
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-falinks-BookSprite.png
DexCategory: "Formation Pok\xE9mon"
DexDescription: 'It is formed by six of them: five are troopers, and one is the brass.
  The brass''s orders are absolute to the others. They work as a perfect team and
  can rearrange their formation to better suit them in battle.'
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Smart Strike
- Close Combat
- Megahorn
- Counter
GenderType: N
GoodStarter: false
Height:
  Feet: 9.8
  Meters: 3.0
HiddenAbility: ''
HomeSprite: SRD-falinks-HomeSprite.png
Image: falinks.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 6
MaxVitality: 6
Name: Falinks
Number: 870
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Rock Smash
- Focus Energy
Special: 2
StandardMoves:
- Headbutt
- Bulk Up
- Helping Hand
- Assurance
StarterMoves:
- Tackle
- Protect
Strength: 3
Type1: Fighting
Type2: ''
Vitality: 3
Weight:
  Kilograms: 62.0
  Pounds: 136.0
---

#PokeroleSRD/Pokedex

# Falinks 

> [!grid]
> ![[SRD-falinks-BookSprite.png|wsmall]]
> ![[SRD-falinks-HomeSprite.png]]

*Formation Pokémon*
*It is formed by six of them: five are troopers, and one is the brass. The brass's orders are absolute to the others. They work as a perfect team and can rearrange their formation to better suit them in battle.*

**DexID**: 0870
**Name**: Falinks
**Type**: Fighting
**Abilities**: [[SRD-Battle Armor|Battle Armor]] / [[SRD-Defiant|Defiant]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 9'8" / 3.0m
**Weight**: 136.0lbs / 62.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets]]