---
Ability1: Sand Stream
Ability2: ''
AceMoves:
- Outrage
- Dragon Dance
- Superpower
- Foul Play
AdvancedMoves:
- Fire Fang
- Thunder Fang
- Ice Fang
- Scary Face
- Thrash
- Stomping Tantrum
- Dark Pulse
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-tyranitar-BookSprite.png
DexCategory: "Armor Pok\xE9mon"
DexDescription: Its body is hardly damaged by any attack, so it's always eager to
  fight. They are extremely strong; their rage can change landscapes. Tyranitar are
  insolents, rebels and they care about no-one.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Pupitar]]'
  Speed: Slow
- Evolves: To
  Item: Tyranitarite
  Kind: Mega
  Pokemon: '[[SRD-Tyranitar (Mega Form)]]'
ExpertMoves:
- Iron Defense
- Crunch
- Payback
- Stone Edge
- Earthquake
- Hyper Beam
- Giga Impact
GenderType: ''
GoodStarter: false
Height:
  Feet: 6.6
  Meters: 2.0
HiddenAbility: Unnerve
HomeSprite: SRD-tyranitar-HomeSprite.png
Image: tyranitar.png
Insight: 3
Legendary: false
MaxDexterity: 5
MaxInsight: 6
MaxSpecial: 6
MaxStrength: 7
MaxVitality: 6
Name: Tyranitar
Number: 248
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Bite
- Rock Throw
Special: 3
StandardMoves:
- Sandstorm
- Screech
- Chip Away
- Rock Slide
StarterMoves:
- Tackle
- Leer
Strength: 3
Type1: Rock
Type2: Dark
Vitality: 3
Weight:
  Kilograms: 202.0
  Pounds: 445.0
---

#PokeroleSRD/Pokedex

# Tyranitar 

> [!grid]
> ![[SRD-tyranitar-BookSprite.png|wsmall]]
> ![[SRD-tyranitar-HomeSprite.png]]

*Armor Pokémon*
*Its body is hardly damaged by any attack, so it's always eager to fight. They are extremely strong; their rage can change landscapes. Tyranitar are insolents, rebels and they care about no-one.*

**DexID**: 0248
**Name**: Tyranitar
**Type**: Rock / Dark
**Abilities**: [[SRD-Sand Stream|Sand Stream]] ([[SRD-Unnerve|Unnerve]])
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 6'6" / 2.0m
**Weight**: 445.0lbs / 202.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon                       | Kind   | Speed   | Item         |
|:----------|:------------------------------|:-------|:--------|:-------------|
| From      | [[SRD-Pupitar]]               | Level  | Slow    |              |
| To        | [[SRD-Tyranitar (Mega Form)]] | Mega   |         | Tyranitarite |


## Learnset

![[Embedded Views.base#Learnsets]]