---
Ability1: Pressure
Ability2: Frisk
AceMoves:
- Imprison
- Pain Split
- Sucker Punch
AdvancedMoves:
- Mean Look
- Will-O-Wisp
- Shadow Punch
- Fire Punch
- Thunder Punch
- Ice Punch
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-dusknoir-BookSprite.png
DexCategory: "Gripper Pok\xE9mon"
DexDescription: "This feared Pok\xE9mon is said to travel to the other world. Some\
  \ even believe that it takes lost spirits along with it. It uses the antenna on\
  \ its head to receive messages from the deceased."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Item: Reaper Cloth
  Kind: Trade
  Pokemon: '[[SRD-Dusclops]]'
ExpertMoves:
- Curse
- Foresight
- Hex
- Shadow Ball
- Dark Pulse
- Payback
- Gravity
- Future Sight
- Destiny Bond
GenderType: ''
GoodStarter: false
Height:
  Feet: 7.3
  Meters: 2.2
HiddenAbility: ''
HomeSprite: SRD-dusknoir-HomeSprite.png
Image: dusknoir.png
Insight: 3
Legendary: false
MaxDexterity: 4
MaxInsight: 8
MaxSpecial: 4
MaxStrength: 6
MaxVitality: 8
Name: Dusknoir
Number: 477
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Disable
- Bind
Special: 2
StandardMoves:
- Confuse Ray
- Shadow Sneak
- Pursuit
- Night Shade
StarterMoves:
- Astonish
- Leer
Strength: 3
Type1: Ghost
Type2: ''
Vitality: 3
Weight:
  Kilograms: 106.0
  Pounds: 235.0
---

#PokeroleSRD/Pokedex

# Dusknoir 

> [!grid]
> ![[SRD-dusknoir-BookSprite.png|wsmall]]
> ![[SRD-dusknoir-HomeSprite.png]]

*Gripper Pokémon*
*This feared Pokémon is said to travel to the other world. Some even believe that it takes lost spirits along with it. It uses the antenna on its head to receive messages from the deceased.*

**DexID**: 0477
**Name**: Dusknoir
**Type**: Ghost
**Abilities**: [[SRD-Pressure|Pressure]] / [[SRD-Frisk|Frisk]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘⭘  | 3/8  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⬤⭘⭘⭘⭘⭘   | 3/8   |

**Height**: 7'3" / 2.2m
**Weight**: 235.0lbs / 106.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon          | Kind   | Item         |
|:----------|:-----------------|:-------|:-------------|
| From      | [[SRD-Dusclops]] | Trade  | Reaper Cloth |


## Learnset

![[Embedded Views.base#Learnsets]]