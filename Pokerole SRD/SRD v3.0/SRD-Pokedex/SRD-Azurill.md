---
Ability1: Huge Power
Ability2: Thick Fat
AceMoves:
- Tickle
- Sing
- Fake Tears
AdvancedMoves:
- Bubble Beam
- Helping Hand
- Bounce
- Soak
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-azurill-BookSprite.png
DexCategory: "Polka Dot Pok\xE9mon"
DexDescription: "They use their tail as a lasso. When they throw their ball, Azurill\
  \ get dragged along with it. They are commonly seen bouncing and playing with other\
  \ Pok\xE9mon on the beach. They love fruit pops."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Stat
  Pokemon: '[[SRD-Marill]]'
  Stat: Happiness
  Value: 4
ExpertMoves: []
GenderType: ''
GoodStarter: true
Height:
  Feet: 0.75
  Meters: 0.2
HiddenAbility: Sap Sipper
HomeSprite: SRD-azurill-HomeSprite.png
Image: azurill.png
Insight: 1
Legendary: false
MaxDexterity: 3
MaxInsight: 3
MaxSpecial: 3
MaxStrength: 3
MaxVitality: 3
Name: Azurill
Number: 298
ProMoves: []
RecommendedRank: Starter
RookieMoves:
- Tail Whip
- Water Sport
Special: 1
StandardMoves:
- Bubble
- Charm
- Slam
- Hold Hands
StarterMoves:
- Splash
- Water Gun
Strength: 1
Type1: Normal
Type2: Fairy
Vitality: 1
Weight:
  Kilograms: 2.0
  Pounds: 4.0
---

#PokeroleSRD/Pokedex

# Azurill 

> [!grid]
> ![[SRD-azurill-BookSprite.png|wsmall]]
> ![[SRD-azurill-HomeSprite.png]]

*Polka Dot Pokémon*
*They use their tail as a lasso. When they throw their ball, Azurill get dragged along with it. They are commonly seen bouncing and playing with other Pokémon on the beach. They love fruit pops.*

**DexID**: 0298
**Name**: Azurill
**Type**: Normal / Fairy
**Abilities**: [[SRD-Huge Power|Huge Power]] / [[SRD-Thick Fat|Thick Fat]] ([[SRD-Sap Sipper|Sap Sipper]])
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⭘⭘  | 1/3  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⭘⭘   | 1/3   |

**Height**: 0'75" / 0.2m
**Weight**: 4.0lbs / 2.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Starter

| Evolves   | Pokemon        | Kind   | Stat      |   Value |
|:----------|:---------------|:-------|:----------|--------:|
| To        | [[SRD-Marill]] | Stat   | Happiness |       4 |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]