---
Ability1: Overgrow
Ability2: Long Reach
AceMoves:
- Haze
- Shadow Claw
- Grass Pledge
AdvancedMoves:
- Synthesis
- Pluck
- Fury Attack
- Nasty Plot
- Sucker Punch
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-dartrix-BookSprite.png
DexCategory: "Blade Quill Pok\xE9mon"
DexDescription: Dartrix are vain creatures. They dislike it when their feathers are
  ruffled, even stopping midfight to groom them. Despite its elegant demeanor, it
  hides an awkward personality, prone to panic and clumsiness.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Rowlet]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Decidueye]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Decidueye (Hisuian Form)]]'
  Region: Hisui
  Speed: Medium
ExpertMoves:
- Leaf Blade
- Feather Dance
- Brave Bird
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.3
  Meters: 0.7
HiddenAbility: ''
HomeSprite: SRD-dartrix-HomeSprite.png
Image: dartrix.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 5
MaxSpecial: 5
MaxStrength: 5
MaxVitality: 5
Name: Dartrix
Number: 723
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Leafage
- Astonish
Special: 2
StandardMoves:
- Peck
- Shadow Sneak
- Foresight
- Razor Leaf
StarterMoves:
- Tackle
- Growl
Strength: 2
Type1: Grass
Type2: Flying
Vitality: 2
Weight:
  Kilograms: 16.0
  Pounds: 35.0
---

#PokeroleSRD/Pokedex

# Dartrix 

> [!grid]
> ![[SRD-dartrix-BookSprite.png|wsmall]]
> ![[SRD-dartrix-HomeSprite.png]]

*Blade Quill Pokémon*
*Dartrix are vain creatures. They dislike it when their feathers are ruffled, even stopping midfight to groom them. Despite its elegant demeanor, it hides an awkward personality, prone to panic and clumsiness.*

**DexID**: 0723
**Name**: Dartrix
**Type**: Grass / Flying
**Abilities**: [[SRD-Overgrow|Overgrow]] / [[SRD-Long Reach|Long Reach]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 2'3" / 0.7m
**Weight**: 35.0lbs / 16.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon                          | Kind   | Speed   | Region   |
|:----------|:---------------------------------|:-------|:--------|:---------|
| From      | [[SRD-Rowlet]]                   | Level  | Medium  |          |
| To        | [[SRD-Decidueye]]                | Level  | Medium  |          |
| To        | [[SRD-Decidueye (Hisuian Form)]] | Level  | Medium  | Hisui    |


## Learnset

![[Embedded Views.base#Learnsets]]