---
Ability1: Ice Body
Ability2: Own Tempo
AceMoves:
- Water Pulse
- Mirror Coat
- Endure
AdvancedMoves:
- Iron Defense
- Recover
- Ice Ball
- Crunch
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-bergmite-BookSprite.png
DexCategory: "Ice Chunk Pok\xE9mon"
DexDescription: It lives in small herds close to the mountains. It blocks attacks
  with the ice that shields its body and uses cold air to repair any cracks with new
  ice. They are wary of humans as they rarely get to see one.
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Avalugg]]'
  Speed: Slow
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Avalugg (Hisuian Form)]]'
  Region: Hisui
  Speed: Slow
ExpertMoves:
- Blizzard
- Double-Edge
GenderType: ''
GoodStarter: true
Height:
  Feet: 3.3
  Meters: 1.0
HiddenAbility: Sturdy
HomeSprite: SRD-bergmite-HomeSprite.png
Image: bergmite.png
Insight: 1
Legendary: false
MaxDexterity: 3
MaxInsight: 3
MaxSpecial: 3
MaxStrength: 4
MaxVitality: 5
Name: Bergmite
Number: 712
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Powder Snow
- Tackle
Special: 1
StandardMoves:
- Curse
- Icy Wind
- Protect
- Take Down
- Bite
- Ice Fang
StarterMoves:
- Rapid Spin
- Harden
- Sharpen
Strength: 2
Type1: Ice
Type2: ''
Vitality: 2
Weight:
  Kilograms: 100.0
  Pounds: 220.0
---

#PokeroleSRD/Pokedex

# Bergmite 

> [!grid]
> ![[SRD-bergmite-BookSprite.png|wsmall]]
> ![[SRD-bergmite-HomeSprite.png]]

*Ice Chunk Pokémon*
*It lives in small herds close to the mountains. It blocks attacks with the ice that shields its body and uses cold air to repair any cracks with new ice. They are wary of humans as they rarely get to see one.*

**DexID**: 0712
**Name**: Bergmite
**Type**: Ice
**Abilities**: [[SRD-Ice Body|Ice Body]] / [[SRD-Own Tempo|Own Tempo]] ([[SRD-Sturdy|Sturdy]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⭘⭘   | 1/3   |

**Height**: 3'3" / 1.0m
**Weight**: 220.0lbs / 100.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon                        | Kind   | Speed   | Region   |
|:----------|:-------------------------------|:-------|:--------|:---------|
| To        | [[SRD-Avalugg]]                | Level  | Slow    |          |
| To        | [[SRD-Avalugg (Hisuian Form)]] | Level  | Slow    | Hisui    |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]