---
Ability1: Gulp Missile
Ability2: ''
AceMoves:
- Hurricane
- Aqua Ring
- Surf
AdvancedMoves:
- Feather Dance
- Roost
- Dive
- Drill Peck
- Amnesia
- Scald
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-cramorant-gulping-form-BookSprite.png
DexCategory: "Gulp Pok\xE9mon"
DexDescription: Cramorant's Gulping Form. It dove in the sea and caught an Arrokuda.
  Can use the Move Bite.
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Agility
- Thrash
- Hydro Pump
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: ''
HomeSprite: SRD-cramorant-gulping-form-HomeSprite.png
Image: cramorant-gulping-form.png
Insight: 3
Legendary: false
MaxDexterity: 5
MaxInsight: 6
MaxSpecial: 5
MaxStrength: 5
MaxVitality: 4
Name: Cramorant (Gulping Form)
Number: 845
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Stockpile
- Swallow
Special: 2
StandardMoves:
- Spit Up
- Water Gun
- Fury Attack
- Pluck
StarterMoves:
- Belch
- Peck
- Bite
Strength: 2
Type1: Flying
Type2: Water
Vitality: 2
Weight:
  Kilograms: 18.0
  Pounds: 40.0
---

#PokeroleSRD/Pokedex

# Cramorant (Gulping Form) 

> [!grid]
> ![[SRD-cramorant-gulping-form-BookSprite.png|wsmall]]
> ![[SRD-cramorant-gulping-form-HomeSprite.png]]

*Gulp Pokémon*
*Cramorant's Gulping Form. It dove in the sea and caught an Arrokuda. Can use the Move Bite.*

**DexID**: 0845F1
**Name**: Cramorant (Gulping Form)
**Type**: Flying / Water
**Abilities**: [[SRD-Gulp Missile|Gulp Missile]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 2'6" / 0.8m
**Weight**: 40.0lbs / 18.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard


## Learnset

![[Embedded Views.base#Learnsets]]