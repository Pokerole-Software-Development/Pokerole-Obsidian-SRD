---
Ability1: Poison Point
Ability2: Swarm
AceMoves:
- Toxic Spikes
- Spikes
- Pin Missile
AdvancedMoves:
- Agility
- Toxic
- Rock Climb
- Venom Drench
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-venipede-BookSprite.png
DexCategory: "Centipede Pok\xE9mon"
DexDescription: "Incredibly aggressive for a Pok\xE9mon that size. It uses the feelers\
  \ on its front and back to locate things around it. Its bite injects a potent venom,\
  \ enough to deter the large bird Pok\xE9mon that try to prey on it."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Whirlipede]]'
  Speed: Medium
ExpertMoves:
- Double-Edge
- Steamroller
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.3
  Meters: 0.4
HiddenAbility: Speed Boost
HomeSprite: SRD-venipede-HomeSprite.png
Image: venipede.png
Insight: 1
Legendary: false
MaxDexterity: 4
MaxInsight: 3
MaxSpecial: 3
MaxStrength: 4
MaxVitality: 4
Name: Venipede
Number: 543
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Rollout
- Pursuit
Special: 1
StandardMoves:
- Protect
- Poison Tail
- Screech
- Bug Bite
- Venoshock
- Take Down
StarterMoves:
- Poison Sting
- Defense Curl
Strength: 2
Type1: Bug
Type2: Poison
Vitality: 2
Weight:
  Kilograms: 5.0
  Pounds: 11.0
---

#PokeroleSRD/Pokedex

# Venipede 

> [!grid]
> ![[SRD-venipede-BookSprite.png|wsmall]]
> ![[SRD-venipede-HomeSprite.png]]

*Centipede Pokémon*
*Incredibly aggressive for a Pokémon that size. It uses the feelers on its front and back to locate things around it. Its bite injects a potent venom, enough to deter the large bird Pokémon that try to prey on it.*

**DexID**: 0543
**Name**: Venipede
**Type**: Bug / Poison
**Abilities**: [[SRD-Poison Point|Poison Point]] / [[SRD-Swarm|Swarm]] ([[SRD-Speed Boost|Speed Boost]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⭘⭘   | 1/3   |

**Height**: 1'3" / 0.4m
**Weight**: 11.0lbs / 5.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Standard

| Evolves   | Pokemon            | Kind   | Speed   |
|:----------|:-------------------|:-------|:--------|
| To        | [[SRD-Whirlipede]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]