---
Ability1: Snow Cloak
Ability2: Ice Body
AceMoves:
- Wish
- Captivate
- Fake Tears
AdvancedMoves:
- Ice Shard
- Barrier
- Ice Fang
- Mirror Coat
- Freeze Dry
- Hail
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-glaceon-BookSprite.png
DexCategory: "Fresh Snow Pok\xE9mon"
DexDescription: Eevee that are forced to live in freezing temperatures evolve into
  Glaceon. It can control its body temperature to below zero, freezing its fur and
  making it harden as if it were diamond crystals.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Item: Ice Stone
  Kind: Stone
  Pokemon: '[[SRD-Eevee]]'
ExpertMoves:
- Last Resort
- Blizzard
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: ''
HomeSprite: SRD-glaceon-HomeSprite.png
Image: glaceon.png
Insight: 3
Legendary: false
MaxDexterity: 4
MaxInsight: 6
MaxSpecial: 7
MaxStrength: 4
MaxVitality: 6
Name: Glaceon
Number: 471
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Tail Whip
- Sand Attack
Special: 3
StandardMoves:
- Icy Wind
- Quick Attack
- Swift
- Bite
StarterMoves:
- Helping Hand
- Tackle
Strength: 2
Type1: Ice
Type2: ''
Vitality: 3
Weight:
  Kilograms: 25.0
  Pounds: 56.0
---

#PokeroleSRD/Pokedex

# Glaceon 

> [!grid]
> ![[SRD-glaceon-BookSprite.png|wsmall]]
> ![[SRD-glaceon-HomeSprite.png]]

*Fresh Snow Pokémon*
*Eevee that are forced to live in freezing temperatures evolve into Glaceon. It can control its body temperature to below zero, freezing its fur and making it harden as if it were diamond crystals.*

**DexID**: 0471
**Name**: Glaceon
**Type**: Ice
**Abilities**: [[SRD-Snow Cloak|Snow Cloak]] / [[SRD-Ice Body|Ice Body]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 2'6" / 0.8m
**Weight**: 56.0lbs / 25.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon       | Kind   | Item      |
|:----------|:--------------|:-------|:----------|
| From      | [[SRD-Eevee]] | Stone  | Ice Stone |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]