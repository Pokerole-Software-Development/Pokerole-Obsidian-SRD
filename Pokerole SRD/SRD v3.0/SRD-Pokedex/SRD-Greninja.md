---
Ability1: Torrent
Ability2: Protean
AceMoves:
- Gunk Shot
- Ice Punch
- Mind Reader
- Hydro Cannon
AdvancedMoves:
- Feint Attack
- Smack Down
- Shadow Sneak
- Spikes
- Extrasensory
- Role Play
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-greninja-BookSprite.png
DexCategory: "Ninja Pok\xE9mon"
DexDescription: It appears and vanishes with a ninja's grace. It toys with its enemies
  using swift movements, then slices them by throwing sharp water stars. If it was
  not properly disciplined, it will never listen any master.
Dexterity: 3
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Frogadier]]'
  Speed: Medium
- Evolves: To
  Kind: Form
  Pokemon: '[[SRD-Greninja (Battle Bond Form)]]'
ExpertMoves:
- Snatch
- Mat Block
- Water Shuriken
- Substitute
- Double Team
- Hydro Pump
- Night Slash
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.0
  Meters: 1.5
HiddenAbility: ''
HomeSprite: SRD-greninja-HomeSprite.png
Image: greninja.png
Insight: 2
Legendary: false
MaxDexterity: 7
MaxInsight: 5
MaxSpecial: 6
MaxStrength: 6
MaxVitality: 4
Name: Greninja
Number: 658
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Bubble
- Quick Attack
Special: 3
StandardMoves:
- Lick
- Water Pulse
- Smokescreen
- Haze
StarterMoves:
- Pound
- Growl
Strength: 3
Type1: Water
Type2: Dark
Vitality: 2
Weight:
  Kilograms: 40.0
  Pounds: 88.0
---

#PokeroleSRD/Pokedex

# Greninja 

> [!grid]
> ![[SRD-greninja-BookSprite.png|wsmall]]
> ![[SRD-greninja-HomeSprite.png]]

*Ninja Pokémon*
*It appears and vanishes with a ninja's grace. It toys with its enemies using swift movements, then slices them by throwing sharp water stars. If it was not properly disciplined, it will never listen any master.*

**DexID**: 0658
**Name**: Greninja
**Type**: Water / Dark
**Abilities**: [[SRD-Torrent|Torrent]] / [[SRD-Protean|Protean]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⬤⭘⭘⭘⭘ | 3/7 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 5'0" / 1.5m
**Weight**: 88.0lbs / 40.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon                             | Kind   | Speed   |
|:----------|:------------------------------------|:-------|:--------|
| From      | [[SRD-Frogadier]]                   | Level  | Medium  |
| To        | [[SRD-Greninja (Battle Bond Form)]] | Form   |         |


## Learnset

![[Embedded Views.base#Learnsets]]