---
Ability1: Intimidate
Ability2: Moxie
AceMoves:
- Shadow Claw
- Superpower
- Dragon Tail
AdvancedMoves:
- Torment
- Mud Slap
- Dig
- Swagger
- Crunch
- Sandstorm
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-krookodile-BookSprite.png
DexCategory: "Intimidation Pok\xE9mon"
DexDescription: "A very violent Pok\xE9mon. They try to clamp down on anything that\
  \ moves in front of their eyes, and once grasped, they never let the prey escape.\
  \ It can be very dangerous if it's not tamed correctly."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Krokorok]]'
  Speed: Medium
ExpertMoves:
- Assurance
- Embargo
- Foul Play
- Lash Out
- Spite
- Thrash
- Earthquake
- Outrage
GenderType: ''
GoodStarter: false
Height:
  Feet: 4.9
  Meters: 1.5
HiddenAbility: Anger Point
HomeSprite: SRD-krookodile-HomeSprite.png
Image: krookodile.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 5
MaxSpecial: 4
MaxStrength: 6
MaxVitality: 5
Name: Krookodile
Number: 553
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Rage
- Sand Attack
Special: 2
StandardMoves:
- Hone Claws
- Sand Tomb
- Scary Face
- Bite
StarterMoves:
- Power Trip
- Leer
Strength: 3
Type1: Ground
Type2: Dark
Vitality: 2
Weight:
  Kilograms: 96.0
  Pounds: 212.0
---

#PokeroleSRD/Pokedex

# Krookodile 

> [!grid]
> ![[SRD-krookodile-BookSprite.png|wsmall]]
> ![[SRD-krookodile-HomeSprite.png]]

*Intimidation Pokémon*
*A very violent Pokémon. They try to clamp down on anything that moves in front of their eyes, and once grasped, they never let the prey escape. It can be very dangerous if it's not tamed correctly.*

**DexID**: 0553
**Name**: Krookodile
**Type**: Ground / Dark
**Abilities**: [[SRD-Intimidate|Intimidate]] / [[SRD-Moxie|Moxie]] ([[SRD-Anger Point|Anger Point]])
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 4'9" / 1.5m
**Weight**: 212.0lbs / 96.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon          | Kind   | Speed   |
|:----------|:-----------------|:-------|:--------|
| From      | [[SRD-Krokorok]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]