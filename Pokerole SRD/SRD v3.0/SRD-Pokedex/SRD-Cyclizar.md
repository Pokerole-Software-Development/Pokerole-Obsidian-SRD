---
Ability1: Shed Skin
Ability2: Regenerator
AceMoves:
- Outrage
- Ice Spinner
- Draco Meteor
AdvancedMoves:
- Shed Tail
- Dragon Claw
- Dragon Tail
- Acrobatics
- Shift Gear
- Dragon Pulse
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-cyclizar-BookSprite.png
DexCategory: "Mount Pok\xE9mon"
DexDescription: 'There are old murals depicting their use as a mount. It is believed
  they allowed humans to ride them due to being cold-blooded: humans served as a source
  of heat. They can go over 70 mph (112 kph).'
Dexterity: 3
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Double-Edge
- Dragon Rush
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.3
  Meters: 1.6
HiddenAbility: ''
HomeSprite: SRD-cyclizar-HomeSprite.png
Image: cyclizar.png
Insight: 2
Legendary: false
MaxDexterity: 7
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 6
MaxVitality: 5
Name: Cyclizar
Number: 967
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Rapid Spin
- Taunt
Special: 2
StandardMoves:
- Breaking Swipe
- Quick Attack
- Bite
- U-Turn
StarterMoves:
- Tackle
- Growl
Strength: 3
Type1: Dragon
Type2: Normal
Vitality: 2
Weight:
  Kilograms: 63.0
  Pounds: 139.0
---

#PokeroleSRD/Pokedex

# Cyclizar 

> [!grid]
> ![[SRD-cyclizar-BookSprite.png|wsmall]]
> ![[SRD-cyclizar-HomeSprite.png]]

*Mount Pokémon*
*There are old murals depicting their use as a mount. It is believed they allowed humans to ride them due to being cold-blooded: humans served as a source of heat. They can go over 70 mph (112 kph).*

**DexID**: 0967
**Name**: Cyclizar
**Type**: Dragon / Normal
**Abilities**: [[SRD-Shed Skin|Shed Skin]] / [[SRD-Regenerator|Regenerator]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⬤⭘⭘⭘⭘ | 3/7 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 5'3" / 1.6m
**Weight**: 139.0lbs / 63.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets]]