---
Ability1: Chlorophyll
Ability2: ''
AceMoves:
- Nature Power
- Heal Pulse
- Weather Ball
AdvancedMoves:
- Sunny Day
- Petal Blizzard
- Worry Seed
- Morning Sun
- Petal Dance
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-cherubi-BookSprite.png
DexCategory: "Cherry Pok\xE9mon"
DexDescription: "It hides inside of bushes to absorb sunlight. Their small head stores\
  \ the energy it needs to evolve, but this small head is frequently eaten by other\
  \ Pok\xE9mon and people, so it's hard for them to evolve."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Cherrim]]'
  Speed: Fast
ExpertMoves:
- Lucky Chant
- Solar Beam
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.3
  Meters: 0.4
HiddenAbility: ''
HomeSprite: SRD-cherubi-HomeSprite.png
Image: cherubi.png
Insight: 2
Legendary: false
MaxDexterity: 3
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 3
MaxVitality: 4
Name: Cherubi
Number: 420
ProMoves: []
RecommendedRank: Starter
RookieMoves:
- Absorb
- Helping Hand
Special: 2
StandardMoves:
- Magical Leaf
- Leech Seed
- Draining Kiss
- Take Down
- Stun Spore
- Sleep Powder
StarterMoves:
- Tackle
- Growth
Strength: 1
Type1: Grass
Type2: ''
Vitality: 2
Weight:
  Kilograms: 3.0
  Pounds: 7.0
---

#PokeroleSRD/Pokedex

# Cherubi 

> [!grid]
> ![[SRD-cherubi-BookSprite.png|wsmall]]
> ![[SRD-cherubi-HomeSprite.png]]

*Cherry Pokémon*
*It hides inside of bushes to absorb sunlight. Their small head stores the energy it needs to evolve, but this small head is frequently eaten by other Pokémon and people, so it's hard for them to evolve.*

**DexID**: 0420
**Name**: Cherubi
**Type**: Grass
**Abilities**: [[SRD-Chlorophyll|Chlorophyll]]
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'3" / 0.4m
**Weight**: 7.0lbs / 3.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Starter

| Evolves   | Pokemon         | Kind   | Speed   |
|:----------|:----------------|:-------|:--------|
| To        | [[SRD-Cherrim]] | Level  | Fast    |


## Learnset

![[Embedded Views.base#Learnsets]]