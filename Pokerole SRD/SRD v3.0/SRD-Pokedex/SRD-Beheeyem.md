---
Ability1: Telepathy
Ability2: Analytic
AceMoves:
- Meteor Beam
- Nasty Plot
- Tri Attack
AdvancedMoves:
- Headbutt
- Zen Headbutt
- Recover
- Calm Mind
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-beheeyem-BookSprite.png
DexCategory: "Cerebral Pok\xE9mon"
DexDescription: Apparently, it communicates by flashing its three fingers, but the
  patterns haven't been decoded. It uses psychic power to control an opponent's brain
  and tampers with its memories and personality.
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Elgyem]]'
  Speed: Medium
ExpertMoves:
- Hidden Power
- Simple Beam
- Psych Up
- Heal Block
- Psychic
- Wonder Room
- Synchronoise
- Psychic Terrain
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.25
  Meters: 1.0
HiddenAbility: ''
HomeSprite: SRD-beheeyem-HomeSprite.png
Image: beheeyem.png
Insight: 3
Legendary: false
MaxDexterity: 3
MaxInsight: 6
MaxSpecial: 7
MaxStrength: 5
MaxVitality: 5
Name: Beheeyem
Number: 606
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Imprison
- Teleport
Special: 3
StandardMoves:
- Miracle Eye
- Psybeam
- Guard Split
- Power Split
StarterMoves:
- Confusion
- Growl
Strength: 2
Type1: Psychic
Type2: ''
Vitality: 2
Weight:
  Kilograms: 34.0
  Pounds: 76.0
---

#PokeroleSRD/Pokedex

# Beheeyem 

> [!grid]
> ![[SRD-beheeyem-BookSprite.png|wsmall]]
> ![[SRD-beheeyem-HomeSprite.png]]

*Cerebral Pokémon*
*Apparently, it communicates by flashing its three fingers, but the patterns haven't been decoded. It uses psychic power to control an opponent's brain and tampers with its memories and personality.*

**DexID**: 0606
**Name**: Beheeyem
**Type**: Psychic
**Abilities**: [[SRD-Telepathy|Telepathy]] / [[SRD-Analytic|Analytic]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 3'25" / 1.0m
**Weight**: 76.0lbs / 34.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon        | Kind   | Speed   |
|:----------|:---------------|:-------|:--------|
| From      | [[SRD-Elgyem]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]