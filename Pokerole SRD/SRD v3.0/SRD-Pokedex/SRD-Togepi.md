---
Ability1: Hustle
Ability2: Serene Grace
AceMoves:
- Present
- Nasty Plot
- Extrasensory
AdvancedMoves:
- Safeguard
- Ancient Power
- Double-Edge
- Last Resort
- Wish
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-togepi-BookSprite.png
DexCategory: "Spike Ball Pok\xE9mon"
DexDescription: "This Pok\xE9mon stores all the kindness and good feelings inside\
  \ of its shell and shares them with others. When the shell is full, it becomes so\
  \ happy that it finally evolves. It is said that it will bring good fortune to its\
  \ trainer."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Stat
  Pokemon: '[[SRD-Togetic]]'
  Stat: Happiness
  Value: 5
ExpertMoves: []
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.0
  Meters: 0.3
HiddenAbility: Super Luck
HomeSprite: SRD-togepi-HomeSprite.png
Image: togepi.png
Insight: 2
Legendary: false
MaxDexterity: 3
MaxInsight: 4
MaxSpecial: 3
MaxStrength: 3
MaxVitality: 4
Name: Togepi
Number: 175
ProMoves: []
RecommendedRank: Starter
RookieMoves:
- Metronome
- Sweet Kiss
- Yawn
- Encore
Special: 1
StandardMoves:
- Life Dew
- Hold Hands
- Follow Me
- After You
- Baton Pass
- Bestow
StarterMoves:
- Pound
- Charm
Strength: 1
Type1: Fairy
Type2: ''
Vitality: 2
Weight:
  Kilograms: 1.5
  Pounds: 3.0
---

#PokeroleSRD/Pokedex

# Togepi 

> [!grid]
> ![[SRD-togepi-BookSprite.png|wsmall]]
> ![[SRD-togepi-HomeSprite.png]]

*Spike Ball Pokémon*
*This Pokémon stores all the kindness and good feelings inside of its shell and shares them with others. When the shell is full, it becomes so happy that it finally evolves. It is said that it will bring good fortune to its trainer.*

**DexID**: 0175
**Name**: Togepi
**Type**: Fairy
**Abilities**: [[SRD-Hustle|Hustle]] / [[SRD-Serene Grace|Serene Grace]] ([[SRD-Super Luck|Super Luck]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'0" / 0.3m
**Weight**: 3.0lbs / 1.5kg
**Good Starter**:: Yes
**Recommended Rank**:: Starter

| Evolves   | Pokemon         | Kind   | Stat      |   Value |
|:----------|:----------------|:-------|:----------|--------:|
| To        | [[SRD-Togetic]] | Stat   | Happiness |       5 |


## Learnset

![[Embedded Views.base#Learnsets]]