---
Ability1: Poison Point
Ability2: Technician
AceMoves:
- Leaf Storm
- Sludge Bomb
- Life Dew
AdvancedMoves:
- Sweet Scent
- Toxic Spikes
- Weather Ball
- Venom Drench
- Grassy Terrain
- Ingrain
- Petal Blizzard
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-roserade-BookSprite.png
DexCategory: "Bouquet Pok\xE9mon"
DexDescription: Luring prey with a sweet scent, it uses the poison on its thorn-filled
  arm-whips to poison, bind, and finish off its prey. It has a dangerous appeal mixed
  with a graceful personality. It's very rare in the wild.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Item: Shiny Stone
  Kind: Stone
  Pokemon: '[[SRD-Roselia]]'
ExpertMoves:
- Toxic
- Aromatherapy
- Poison Jab
- Giga Drain
- Synthesis
- Petal Dance
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.0
  Meters: 0.9
HiddenAbility: ''
HomeSprite: SRD-roserade-HomeSprite.png
Image: roserade.png
Insight: 3
Legendary: false
MaxDexterity: 5
MaxInsight: 6
MaxSpecial: 7
MaxStrength: 5
MaxVitality: 4
Name: Roserade
Number: 407
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Poison Sting
- Stun Spore
Special: 3
StandardMoves:
- Mega Drain
- Leech Seed
- Magical Leaf
- Grass Whistle
StarterMoves:
- Absorb
- Growth
Strength: 2
Type1: Grass
Type2: Poison
Vitality: 2
Weight:
  Kilograms: 30.0
  Pounds: 66.0
---

#PokeroleSRD/Pokedex

# Roserade 

> [!grid]
> ![[SRD-roserade-BookSprite.png|wsmall]]
> ![[SRD-roserade-HomeSprite.png]]

*Bouquet Pokémon*
*Luring prey with a sweet scent, it uses the poison on its thorn-filled arm-whips to poison, bind, and finish off its prey. It has a dangerous appeal mixed with a graceful personality. It's very rare in the wild.*

**DexID**: 0407
**Name**: Roserade
**Type**: Grass / Poison
**Abilities**: [[SRD-Poison Point|Poison Point]] / [[SRD-Technician|Technician]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 3'0" / 0.9m
**Weight**: 66.0lbs / 30.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon         | Kind   | Item        |
|:----------|:----------------|:-------|:------------|
| From      | [[SRD-Roselia]] | Stone  | Shiny Stone |


## Learnset

![[Embedded Views.base#Learnsets]]