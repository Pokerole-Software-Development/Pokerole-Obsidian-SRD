---
Ability1: Overcoat
Ability2: Soundproof
AceMoves:
- Counter
- Dragon Breath
- Uproar
AdvancedMoves:
- Work Up
- Screech
- Iron Defense
- Autotomize
- Dragon Dance
- Dragon Claw
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-hakamo-o-BookSprite.png
DexCategory: "Scaly Pok\xE9mon"
DexDescription: "The scales on its body are shed and regrow sharper each time. It\
  \ is a courageous and diligent Pok\xE9mon who always lets out a war cry before the\
  \ battle. Any marks on its scale armor is worn with pride."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Jangmo-o]]'
  Speed: Slow
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Kommo-o]]'
  Speed: Slow
ExpertMoves:
- Scale Shot
- Sky Uppercut
- Outrage
- Close Combat
GenderType: ''
GoodStarter: false
Height:
  Feet: 4.0
  Meters: 1.2
HiddenAbility: ''
HomeSprite: SRD-hakamo-o-HomeSprite.png
Image: hakamo-o.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 5
MaxSpecial: 4
MaxStrength: 5
MaxVitality: 5
Name: Hakamo-o
Number: 783
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Bide
- Protect
Special: 2
StandardMoves:
- Dragon Tail
- Noble Roar
- Scary Face
- Headbutt
StarterMoves:
- Tackle
- Leer
Strength: 2
Type1: Dragon
Type2: Fighting
Vitality: 2
Weight:
  Kilograms: 47.0
  Pounds: 103.0
---

#PokeroleSRD/Pokedex

# Hakamo-o 

> [!grid]
> ![[SRD-hakamo-o-BookSprite.png|wsmall]]
> ![[SRD-hakamo-o-HomeSprite.png]]

*Scaly Pokémon*
*The scales on its body are shed and regrow sharper each time. It is a courageous and diligent Pokémon who always lets out a war cry before the battle. Any marks on its scale armor is worn with pride.*

**DexID**: 0783
**Name**: Hakamo-o
**Type**: Dragon / Fighting
**Abilities**: [[SRD-Overcoat|Overcoat]] / [[SRD-Soundproof|Soundproof]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 4'0" / 1.2m
**Weight**: 103.0lbs / 47.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon          | Kind   | Speed   |
|:----------|:-----------------|:-------|:--------|
| From      | [[SRD-Jangmo-o]] | Level  | Slow    |
| To        | [[SRD-Kommo-o]]  | Level  | Slow    |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]