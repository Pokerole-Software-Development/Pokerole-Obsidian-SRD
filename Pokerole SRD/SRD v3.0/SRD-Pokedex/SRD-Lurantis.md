---
Ability1: Leaf Guard
Ability2: Contrary
AceMoves:
- Leaf Storm
- Cross Poison
- Swords Dance
AdvancedMoves:
- Weather Ball
- Swagger
- Sunny Day
- Aromatherapy
- Brick Break
- Synthesis
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-lurantis-BookSprite.png
DexCategory: "Bloom Sickle Pok\xE9mon"
DexDescription: "Considered to be one of the most beautiful Grass-Type Pok\xE9mon\
  \ due to its lovely coloration. They require a lot of maintenance and constant grooming,\
  \ so they'll only trust a Trainer who is up for the job."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Fomantis]]'
  Speed: Medium
ExpertMoves:
- Leaf Blade
- Night Slash
- Dual Chop
- X-Scissor
- Solar Blade
- Solar Beam
- Petal Blizzard
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.0
  Meters: 0.9
HiddenAbility: ''
HomeSprite: SRD-lurantis-HomeSprite.png
Image: lurantis.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 5
MaxSpecial: 5
MaxStrength: 6
MaxVitality: 5
Name: Lurantis
Number: 754
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Razor Leaf
- Growth
Special: 2
StandardMoves:
- Ingrain
- Sweet Scent
- Slash
- Worry Seed
StarterMoves:
- Fury Cutter
- Leafage
Strength: 3
Type1: Grass
Type2: ''
Vitality: 2
Weight:
  Kilograms: 18.0
  Pounds: 40.0
---

#PokeroleSRD/Pokedex

# Lurantis 

> [!grid]
> ![[SRD-lurantis-BookSprite.png|wsmall]]
> ![[SRD-lurantis-HomeSprite.png]]

*Bloom Sickle Pokémon*
*Considered to be one of the most beautiful Grass-Type Pokémon due to its lovely coloration. They require a lot of maintenance and constant grooming, so they'll only trust a Trainer who is up for the job.*

**DexID**: 0754
**Name**: Lurantis
**Type**: Grass
**Abilities**: [[SRD-Leaf Guard|Leaf Guard]] / [[SRD-Contrary|Contrary]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 3'0" / 0.9m
**Weight**: 40.0lbs / 18.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon          | Kind   | Speed   |
|:----------|:-----------------|:-------|:--------|
| From      | [[SRD-Fomantis]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]