---
Ability1: Regenerator
Ability2: Own Tempo
AceMoves:
- Aqua Tail
- Belly Drum
- Future Sight
AdvancedMoves:
- Water Pulse
- Zen Headbutt
- Slack Off
- Withdraw
- Amnesia
- Psychic
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-slowbro-BookSprite.png
DexCategory: "Hermit Crab Pok\xE9mon"
DexDescription: "This Pok\xE9mon fused with a Shellder that bit into its tail. It's\
  \ a slow swimmer and doesn't react to pain, but Shellder tends to keep it out of\
  \ trouble."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Special
  Pokemon: '[[SRD-Slowpoke]]'
  Special: Shellder Biting Tail
- Evolves: To
  Item: Slowbronite
  Kind: Mega
  Pokemon: '[[SRD-Slowbro (Mega Form)]]'
ExpertMoves:
- Surf
- Rain Dance
- Psych Up
- Heal Pulse
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.3
  Meters: 1.6
HiddenAbility: ''
HomeSprite: SRD-slowbro-HomeSprite.png
Image: slowbro.png
Insight: 2
Legendary: false
MaxDexterity: 3
MaxInsight: 5
MaxSpecial: 6
MaxStrength: 4
MaxVitality: 6
Name: Slowbro
Number: 80
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Curse
- Growl
Special: 3
StandardMoves:
- Water Gun
- Confusion
- Disable
- Headbutt
StarterMoves:
- Tackle
- Yawn
Strength: 2
Type1: Water
Type2: Psychic
Vitality: 3
Weight:
  Kilograms: 78.0
  Pounds: 173.0
---

#PokeroleSRD/Pokedex

# Slowbro 

> [!grid]
> ![[SRD-slowbro-BookSprite.png|wsmall]]
> ![[SRD-slowbro-HomeSprite.png]]

*Hermit Crab Pokémon*
*This Pokémon fused with a Shellder that bit into its tail. It's a slow swimmer and doesn't react to pain, but Shellder tends to keep it out of trouble.*

**DexID**: 0080
**Name**: Slowbro
**Type**: Water / Psychic
**Abilities**: [[SRD-Regenerator|Regenerator]] / [[SRD-Own Tempo|Own Tempo]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 5'3" / 1.6m
**Weight**: 173.0lbs / 78.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon                     | Kind    | Special              | Item        |
|:----------|:----------------------------|:--------|:---------------------|:------------|
| From      | [[SRD-Slowpoke]]            | Special | Shellder Biting Tail |             |
| To        | [[SRD-Slowbro (Mega Form)]] | Mega    |                      | Slowbronite |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]