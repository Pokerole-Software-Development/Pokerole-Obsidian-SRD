---
Ability1: Turboblaze
Ability2: ''
AceMoves: []
AdvancedMoves: []
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-reshiram-BookSprite.png
DexCategory: "Pok\xE9dex has no data"
DexDescription: An incredibly old scroll written in an ancient language tells about
  a world of truth built by a purifying fire. It also tells about a big dispute between
  two brothers. The rest of the scroll was burned...
Dexterity: 5
EventAbilities: ''
Evolutions: []
ExpertMoves: []
GenderType: N
GoodStarter: false
Height:
  Feet: 10.5
  Meters: 3.2
HiddenAbility: ''
HomeSprite: SRD-reshiram-HomeSprite.png
Image: reshiram.png
Insight: 7
Legendary: true
MaxDexterity: 5
MaxInsight: 7
MaxSpecial: 8
MaxStrength: 7
MaxVitality: 6
Name: Reshiram
Number: 643
ProMoves: []
RecommendedRank: Master
RookieMoves: []
Special: 8
StandardMoves: []
StarterMoves: []
Strength: 7
Type1: Dragon
Type2: Fire
Vitality: 6
Weight:
  Kilograms: 330.0
  Pounds: 727.0
---

#PokeroleSRD/Pokedex

# Reshiram 

> [!grid]
> ![[SRD-reshiram-BookSprite.png|wsmall]]
> ![[SRD-reshiram-HomeSprite.png]]

*Pokédex has no data*
*An incredibly old scroll written in an ancient language tells about a world of truth built by a purifying fire. It also tells about a big dispute between two brothers. The rest of the scroll was burned...*

**DexID**: 0643
**Name**: Reshiram
**Type**: Dragon / Fire
**Abilities**: [[SRD-Turboblaze|Turboblaze]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⬤⬤⬤⬤  | 7/7  |
| Dexterity | ⬤⬤⬤⬤⬤ | 5/5 |
| Vitality  | ⬤⬤⬤⬤⬤⬤  | 6/6  |
| Special   | ⬤⬤⬤⬤⬤⬤⬤⬤   | 8/8   |
| Insight   | ⬤⬤⬤⬤⬤⬤⬤   | 7/7   |

**Height**: 10'5" / 3.2m
**Weight**: 727.0lbs / 330.0kg
**Good Starter**:: No
**Recommended Rank**:: Master


## Learnset

![[Embedded Views.base#Learnsets]]