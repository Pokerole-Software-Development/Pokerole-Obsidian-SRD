---
Ability1: Prankster
Ability2: Chlorophyll
AceMoves:
- Beat Up
- Fake Tears
- Substitute
AdvancedMoves:
- Cotton Spore
- Energy Ball
- Sunny Day
- Cotton Guard
- Endeavor
- Giga Drain
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-cottonee-BookSprite.png
DexCategory: "Cotton Puff Pok\xE9mon"
DexDescription: They go wherever the wind takes them. On rainy days they can't float,
  so they take shelter beneath big trees. To defend from predators, they shed their
  cotton and leave it as a decoy while they escape.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Item: Sun Stone
  Kind: Stone
  Pokemon: '[[SRD-Whimsicott]]'
ExpertMoves:
- Solar Beam
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.0
  Meters: 0.3
HiddenAbility: Infiltrator
HomeSprite: SRD-cottonee-HomeSprite.png
Image: cottonee.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 3
MaxStrength: 3
MaxVitality: 4
Name: Cottonee
Number: 546
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Fairy Wind
- Stun Spore
Special: 1
StandardMoves:
- Mega Drain
- Razor Leaf
- Growth
- Poison Powder
- Charm
- Leech Seed
StarterMoves:
- Absorb
- Helping Hand
Strength: 1
Type1: Grass
Type2: Fairy
Vitality: 2
Weight:
  Kilograms: 0.6
  Pounds: 1.0
---

#PokeroleSRD/Pokedex

# Cottonee 

> [!grid]
> ![[SRD-cottonee-BookSprite.png|wsmall]]
> ![[SRD-cottonee-HomeSprite.png]]

*Cotton Puff Pokémon*
*They go wherever the wind takes them. On rainy days they can't float, so they take shelter beneath big trees. To defend from predators, they shed their cotton and leave it as a decoy while they escape.*

**DexID**: 0546
**Name**: Cottonee
**Type**: Grass / Fairy
**Abilities**: [[SRD-Prankster|Prankster]] / [[SRD-Chlorophyll|Chlorophyll]] ([[SRD-Infiltrator|Infiltrator]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'0" / 0.3m
**Weight**: 1.0lbs / 0.6kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon            | Kind   | Item      |
|:----------|:-------------------|:-------|:----------|
| To        | [[SRD-Whimsicott]] | Stone  | Sun Stone |


## Learnset

![[Embedded Views.base#Learnsets]]