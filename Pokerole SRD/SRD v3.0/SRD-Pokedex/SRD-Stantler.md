---
Ability1: Frisk
Ability2: Intimidate
AceMoves:
- Thrash
- Bounce
- Psyshield Bash
AdvancedMoves:
- Jump Kick
- Lunge
- Imprison
- Entrainment
- Captivate
- Disable
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-stantler-BookSprite.png
DexCategory: "Big Horn Pok\xE9mon"
DexDescription: Their antlers create a distortion in space that causes confusion.
  They were hunted for their magnificent antlers, that were sold at high prices, nearly
  driving Stantler extinct. Their antlers don't grow as big as a result.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Special
  Pokemon: '[[SRD-Wyrdeer]]'
  Special: Learning Psyshield Bash
ExpertMoves:
- Light Screen
- Reflect
- Double-Edge
- Megahorn
GenderType: ''
GoodStarter: false
Height:
  Feet: 4.6
  Meters: 1.4
HiddenAbility: Sap Sipper
HomeSprite: SRD-stantler-HomeSprite.png
Image: stantler.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 6
MaxVitality: 4
Name: Stantler
Number: 234
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Astonish
- Sand Attack
Special: 2
StandardMoves:
- Hypnosis
- Stomp
- Take Down
- Me First
- Confuse Ray
- Calm Mind
- Role Play
- Zen Headbutt
StarterMoves:
- Tackle
- Leer
Strength: 3
Type1: Normal
Type2: ''
Vitality: 2
Weight:
  Kilograms: 141.0
  Pounds: 313.0
---

#PokeroleSRD/Pokedex

# Stantler 

> [!grid]
> ![[SRD-stantler-BookSprite.png|wsmall]]
> ![[SRD-stantler-HomeSprite.png]]

*Big Horn Pokémon*
*Their antlers create a distortion in space that causes confusion. They were hunted for their magnificent antlers, that were sold at high prices, nearly driving Stantler extinct. Their antlers don't grow as big as a result.*

**DexID**: 0234
**Name**: Stantler
**Type**: Normal
**Abilities**: [[SRD-Frisk|Frisk]] / [[SRD-Intimidate|Intimidate]] ([[SRD-Sap Sipper|Sap Sipper]])
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 4'6" / 1.4m
**Weight**: 313.0lbs / 141.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon         | Kind    | Special                 |
|:----------|:----------------|:--------|:------------------------|
| To        | [[SRD-Wyrdeer]] | Special | Learning Psyshield Bash |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]