---
Ability1: Swarm
Ability2: Tinted Lens
AceMoves:
- Bounce
- U-Turn
- Facade
AdvancedMoves:
- Feint
- Agility
- Sucker Punch
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-nymble-BookSprite.png
DexCategory: "Grasshopper Pok\xE9mon"
DexDescription: It is very small in size but it has a third set of legs that allow
  it to jump up to 30 feet (9 m) in the air in a pinch. It is also a very capable
  battler who only escapes from a fight if there's really no other choice.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Lokix]]'
  Speed: Medium
ExpertMoves:
- First Impression
GenderType: ''
GoodStarter: true
Height:
  Feet: 0.7
  Meters: 0.2
HiddenAbility: ''
HomeSprite: SRD-nymble-HomeSprite.png
Image: nymble.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 4
Name: Nymble
Number: 919
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Struggle Bug
- Astonish
Special: 2
StandardMoves:
- Assurance
- Double Kick
- Screech
- Endure
- Bug Bite
StarterMoves:
- Tackle
- Leer
Strength: 2
Type1: Bug
Type2: ''
Vitality: 2
Weight:
  Kilograms: 1.0
  Pounds: 2.0
---

#PokeroleSRD/Pokedex

# Nymble 

> [!grid]
> ![[SRD-nymble-BookSprite.png|wsmall]]
> ![[SRD-nymble-HomeSprite.png]]

*Grasshopper Pokémon*
*It is very small in size but it has a third set of legs that allow it to jump up to 30 feet (9 m) in the air in a pinch. It is also a very capable battler who only escapes from a fight if there's really no other choice.*

**DexID**: 0919
**Name**: Nymble
**Type**: Bug
**Abilities**: [[SRD-Swarm|Swarm]] / [[SRD-Tinted Lens|Tinted Lens]]
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 0'7" / 0.2m
**Weight**: 2.0lbs / 1.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon       | Kind   | Speed   |
|:----------|:--------------|:-------|:--------|
| To        | [[SRD-Lokix]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]