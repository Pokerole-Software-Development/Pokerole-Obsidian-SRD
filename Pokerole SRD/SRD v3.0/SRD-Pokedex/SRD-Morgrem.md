---
Ability1: Prankster
Ability2: Frisk
AceMoves:
- Metronome
- Burning Jealousy
- Drain Punch
AdvancedMoves:
- Taunt
- Trick
- False Surrender
- Shadow Claw
- Torment
- Dark Pulse
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-morgrem-BookSprite.png
DexCategory: "Devious Pok\xE9mon"
DexDescription: When it gets down on all fours as if to beg for forgiveness, it's
  trying to lure opponents in so that it can stab them with its spear-like hair. It
  lures people into the woods to rob them and get them lost.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Impidimp]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Grimmsnarl]]'
  Speed: Medium
ExpertMoves:
- Nasty Plot
- Revenge
- Play Rough
- Foul Play
GenderType: M
GoodStarter: false
Height:
  Feet: 2.6
  Meters: 0.8
HiddenAbility: Pickpocket
HomeSprite: SRD-morgrem-HomeSprite.png
Image: morgrem.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 4
MaxVitality: 4
Name: Morgrem
Number: 860
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Bite
- Flatter
Special: 2
StandardMoves:
- Fake Tears
- Assurance
- Swagger
- Sucker Punch
StarterMoves:
- Fake Out
- Confide
Strength: 2
Type1: Dark
Type2: Fairy
Vitality: 2
Weight:
  Kilograms: 12.0
  Pounds: 27.0
---

#PokeroleSRD/Pokedex

# Morgrem 

> [!grid]
> ![[SRD-morgrem-BookSprite.png|wsmall]]
> ![[SRD-morgrem-HomeSprite.png]]

*Devious Pokémon*
*When it gets down on all fours as if to beg for forgiveness, it's trying to lure opponents in so that it can stab them with its spear-like hair. It lures people into the woods to rob them and get them lost.*

**DexID**: 0860
**Name**: Morgrem
**Type**: Dark / Fairy
**Abilities**: [[SRD-Prankster|Prankster]] / [[SRD-Frisk|Frisk]] ([[SRD-Pickpocket|Pickpocket]])
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'6" / 0.8m
**Weight**: 27.0lbs / 12.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon            | Kind   | Speed   |
|:----------|:-------------------|:-------|:--------|
| From      | [[SRD-Impidimp]]   | Level  | Medium  |
| To        | [[SRD-Grimmsnarl]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]