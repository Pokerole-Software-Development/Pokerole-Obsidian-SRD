---
Ability1: Swarm
Ability2: Tinted Lens
AceMoves:
- Swords Dance
- X-Scissor
- Giga Impact
AdvancedMoves:
- Bug Bite
- Feint
- Low Kick
- Detect
- Agility
- Lunge
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-lokix-BookSprite.png
DexCategory: "Grasshopper Pok\xE9mon"
DexDescription: "If this Pok\xE9mon unfolds the legs on its back it means it will\
  \ battle going all-out. When they do this, the battle doesn't usually last long.\
  \ If a large swarm gathers, it won't be good for the crops in the area."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Nymble]]'
  Speed: Medium
ExpertMoves:
- Throat Chop
- Sucker Punch
- First Impression
- Bounce
- Axe Kick
GenderType: ''
GoodStarter: false
Height:
  Feet: 3.3
  Meters: 1.0
HiddenAbility: ''
HomeSprite: SRD-lokix-HomeSprite.png
Image: lokix.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 6
MaxVitality: 5
Name: Lokix
Number: 920
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Struggle Bug
- Astonish
Special: 2
StandardMoves:
- Assurance
- Double Kick
- Screech
- Endure
StarterMoves:
- Tackle
- Leer
Strength: 3
Type1: Bug
Type2: Dark
Vitality: 2
Weight:
  Kilograms: 17.0
  Pounds: 38.0
---

#PokeroleSRD/Pokedex

# Lokix 

> [!grid]
> ![[SRD-lokix-BookSprite.png|wsmall]]
> ![[SRD-lokix-HomeSprite.png]]

*Grasshopper Pokémon*
*If this Pokémon unfolds the legs on its back it means it will battle going all-out. When they do this, the battle doesn't usually last long. If a large swarm gathers, it won't be good for the crops in the area.*

**DexID**: 0920
**Name**: Lokix
**Type**: Bug / Dark
**Abilities**: [[SRD-Swarm|Swarm]] / [[SRD-Tinted Lens|Tinted Lens]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 3'3" / 1.0m
**Weight**: 38.0lbs / 17.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon        | Kind   | Speed   |
|:----------|:---------------|:-------|:--------|
| From      | [[SRD-Nymble]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]