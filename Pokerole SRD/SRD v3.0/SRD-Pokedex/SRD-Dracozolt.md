---
Ability1: Volt Absorb
Ability2: Sand Rush
AceMoves:
- High Horsepower
- Thunder
- Draco Meteor
AdvancedMoves:
- Slam
- Discharge
- Thunder Fang
- Stone Edge
- Wild Charge
- Breaking Swipe
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-dracozolt-BookSprite.png
DexCategory: "Fossil Pok\xE9mon"
DexDescription: "A mix-up of DNA during the revival process resulted in this. Its\
  \ lower body was an incredibly strong Pok\xE9mon that probably preyed on the little\
  \ one from the upper body. Sometimes it will try to run from itself."
Dexterity: 2
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Bolt Beak
- Dragon Pulse
- Dragon Rush
GenderType: N
GoodStarter: false
Height:
  Feet: 5.9
  Meters: 1.8
HiddenAbility: ''
HomeSprite: SRD-dracozolt-HomeSprite.png
Image: dracozolt.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 5
MaxSpecial: 5
MaxStrength: 6
MaxVitality: 5
Name: Dracozolt
Number: 880
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Charge
- Aerial Ace
Special: 2
StandardMoves:
- Ancient Power
- Pluck
- Dragon Tail
- Stomp
StarterMoves:
- Tackle
- Thunder Shock
Strength: 3
Type1: Electric
Type2: Dragon
Vitality: 2
Weight:
  Kilograms: 190.0
  Pounds: 418.0
---

#PokeroleSRD/Pokedex

# Dracozolt 

> [!grid]
> ![[SRD-dracozolt-BookSprite.png|wsmall]]
> ![[SRD-dracozolt-HomeSprite.png]]

*Fossil Pokémon*
*A mix-up of DNA during the revival process resulted in this. Its lower body was an incredibly strong Pokémon that probably preyed on the little one from the upper body. Sometimes it will try to run from itself.*

**DexID**: 0880
**Name**: Dracozolt
**Type**: Electric / Dragon
**Abilities**: [[SRD-Volt Absorb|Volt Absorb]] / [[SRD-Sand Rush|Sand Rush]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 5'9" / 1.8m
**Weight**: 418.0lbs / 190.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced


## Learnset

![[Embedded Views.base#Learnsets]]