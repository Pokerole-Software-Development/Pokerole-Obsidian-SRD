---
Ability1: Chlorophyll
Ability2: Effect Spore
AceMoves:
- Corrosive Gas
- Teeter Dance
- Nature Power
AdvancedMoves:
- Mega Drain
- Moonlight
- Natural Gift
- Toxic
- Giga Drain
- Petal Dance
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-vileplume-BookSprite.png
DexCategory: "Flower Pok\xE9mon"
DexDescription: It lives in marshlands and jungles where it's often mistaken for local
  flora. The air around a Vileplume turns yellow with the powder it releases as it
  walks. The pollen is highly toxic and causes paralysis.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Item: Leaf Stone
  Kind: Stone
  Pokemon: '[[SRD-Gloom]]'
ExpertMoves:
- Moonblast
- Grassy Terrain
- Petal Blizzard
- Solar Beam
- Grassy Glide
- Gastro Acid
GenderType: ''
GoodStarter: false
Height:
  Feet: 4.0
  Meters: 1.2
HiddenAbility: ''
HomeSprite: SRD-vileplume-HomeSprite.png
Image: vileplume.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 5
MaxSpecial: 6
MaxStrength: 5
MaxVitality: 5
Name: Vileplume
Number: 45
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Sweet Scent
- Acid
Special: 3
StandardMoves:
- Poison Powder
- Stun Spore
- Sleep Powder
StarterMoves:
- Absorb
- Growth
Strength: 2
Type1: Grass
Type2: Poison
Vitality: 2
Weight:
  Kilograms: 19.0
  Pounds: 41.0
---

#PokeroleSRD/Pokedex

# Vileplume 

> [!grid]
> ![[SRD-vileplume-BookSprite.png|wsmall]]
> ![[SRD-vileplume-HomeSprite.png]]

*Flower Pokémon*
*It lives in marshlands and jungles where it's often mistaken for local flora. The air around a Vileplume turns yellow with the powder it releases as it walks. The pollen is highly toxic and causes paralysis.*

**DexID**: 0045
**Name**: Vileplume
**Type**: Grass / Poison
**Abilities**: [[SRD-Chlorophyll|Chlorophyll]] / [[SRD-Effect Spore|Effect Spore]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 4'0" / 1.2m
**Weight**: 41.0lbs / 19.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon       | Kind   | Item       |
|:----------|:--------------|:-------|:-----------|
| From      | [[SRD-Gloom]] | Stone  | Leaf Stone |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]