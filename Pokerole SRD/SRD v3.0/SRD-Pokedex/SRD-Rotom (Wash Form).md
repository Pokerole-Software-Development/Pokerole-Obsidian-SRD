---
Ability1: Levitate
Ability2: ''
AceMoves:
- Nasty Plot
- Poltergeist
- Volt Switch
- Eerie Impulse
AdvancedMoves:
- Hex
- Substitute
- Trick
- Discharge
- Thunderbolt
- Shadow Ball
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-rotom-wash-form-BookSprite.png
DexCategory: "Plasma Pok\xE9mon"
DexDescription: A washing machine possessed by a Rotom. It uses Water powers and floods
  houses with torrents of water. While in this form it can use the move 'Hydro Pump'.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Form
  Pokemon: '[[SRD-Rotom]]'
ExpertMoves:
- Uproar
- Dark Pulse
- Thunder
GenderType: N
GoodStarter: false
Height:
  Feet: 1.0
  Meters: 0.3
HiddenAbility: ''
HomeSprite: SRD-rotom-wash-form-HomeSprite.png
Image: rotom-wash-form.png
Insight: 3
Legendary: false
MaxDexterity: 5
MaxInsight: 6
MaxSpecial: 6
MaxStrength: 4
MaxVitality: 6
Name: Rotom (Wash Form)
Number: 479
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Thunder Shock
- Confuse Ray
Special: 3
StandardMoves:
- Double Team
- Charge
- Electro Ball
- Shock Wave
StarterMoves:
- Astonish
- Thunder Wave
- Hydro Pump
Strength: 2
Type1: Electric
Type2: Water
Vitality: 3
Weight:
  Kilograms: 0.3
  Pounds: 0.7
---

#PokeroleSRD/Pokedex

# Rotom (Wash Form) 

> [!grid]
> ![[SRD-rotom-wash-form-BookSprite.png|wsmall]]
> ![[SRD-rotom-wash-form-HomeSprite.png]]

*Plasma Pokémon*
*A washing machine possessed by a Rotom. It uses Water powers and floods houses with torrents of water. While in this form it can use the move 'Hydro Pump'.*

**DexID**: 0479F5
**Name**: Rotom (Wash Form)
**Type**: Electric / Water
**Abilities**: [[SRD-Levitate|Levitate]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 1'0" / 0.3m
**Weight**: 0.7lbs / 0.3kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon       | Kind   |
|:----------|:--------------|:-------|
| From      | [[SRD-Rotom]] | Form   |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]