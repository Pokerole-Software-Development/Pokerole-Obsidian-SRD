---
Ability1: Hunger Switch
Ability2: ''
AceMoves:
- Outrage
- Wild Charge
- Fire Fang
AdvancedMoves:
- Fake Out
- Thunder Fang
- Psychic Fangs
- Parting Shot
- Torment
- Agility
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-morpeko-BookSprite.png
DexCategory: "Two-Sided Pok\xE9mon"
DexDescription: 'Its ''Full-Belly'' Mode is friendly and playful: it treasures food
  and treats. But as its hunger increases it goes into ''Hangry'' Form and starts
  getting violent: the electricity in its cheek sacs turns into a Dark-type energy.'
Dexterity: 3
EventAbilities: ''
Evolutions: []
ExpertMoves:
- Bullet Seed
- Super Fang
- Stomping Tantrum
- Crunch
- Aura Wheel
- Thrash
GenderType: ''
GoodStarter: false
Height:
  Feet: 1.0
  Meters: 0.3
HiddenAbility: ''
HomeSprite: SRD-morpeko-HomeSprite.png
Image: morpeko.png
Insight: 2
Legendary: false
MaxDexterity: 6
MaxInsight: 4
MaxSpecial: 5
MaxStrength: 6
MaxVitality: 4
Name: Morpeko
Number: 877
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Leer
- Power Trip
Special: 2
StandardMoves:
- Quick Attack
- Flatter
- Bite
- Spark
- Quash
- Taunt
StarterMoves:
- Thunder Shock
- Tail Whip
Strength: 3
Type1: Electric
Type2: Dark
Vitality: 2
Weight:
  Kilograms: 3.0
  Pounds: 6.0
---

#PokeroleSRD/Pokedex

# Morpeko 

> [!grid]
> ![[SRD-morpeko-BookSprite.png|wsmall]]
> ![[SRD-morpeko-HomeSprite.png]]

*Two-Sided Pokémon*
*Its 'Full-Belly' Mode is friendly and playful: it treasures food and treats. But as its hunger increases it goes into 'Hangry' Form and starts getting violent: the electricity in its cheek sacs turns into a Dark-type energy.*

**DexID**: 0877
**Name**: Morpeko
**Type**: Electric / Dark
**Abilities**: [[SRD-Hunger Switch|Hunger Switch]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Dexterity | ⬤⬤⬤⭘⭘⭘ | 3/6 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘⭘   | 2/5   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'0" / 0.3m
**Weight**: 6.0lbs / 3.0kg
**Good Starter**:: No
**Recommended Rank**:: Rookie


## Learnset

![[Embedded Views.base#Learnsets v3.0]]