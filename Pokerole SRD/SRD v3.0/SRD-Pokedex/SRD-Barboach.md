---
Ability1: Oblivious
Ability2: Anticipation
AceMoves:
- Dive
- Protect
- Bounce
AdvancedMoves:
- Aqua Tail
- Muddy Water
- Earthquake
- Future Sight
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-barboach-BookSprite.png
DexCategory: "Whiskers Pok\xE9mon"
DexDescription: Their whiskers work as a super sensitive radar. They hide inside the
  mud, leaving only their whiskers exposed while waiting for prey to come. If the
  mud dries, they move back into the water.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Whiscash]]'
  Speed: Medium
ExpertMoves:
- Fissure
GenderType: ''
GoodStarter: true
Height:
  Feet: 1.3
  Meters: 0.4
HiddenAbility: Hydration
HomeSprite: SRD-barboach-HomeSprite.png
Image: barboach.png
Insight: 1
Legendary: false
MaxDexterity: 4
MaxInsight: 3
MaxSpecial: 4
MaxStrength: 4
MaxVitality: 3
Name: Barboach
Number: 339
ProMoves: []
RecommendedRank: Starter
RookieMoves:
- Water Sport
- Water Gun
Special: 2
StandardMoves:
- Mud Bomb
- Amnesia
- Water Pulse
- Magnitude
- Rest
- Snore
StarterMoves:
- Mud Slap
- Mud Sport
Strength: 2
Type1: Water
Type2: Ground
Vitality: 1
Weight:
  Kilograms: 2.0
  Pounds: 5.0
---

#PokeroleSRD/Pokedex

# Barboach 

> [!grid]
> ![[SRD-barboach-BookSprite.png|wsmall]]
> ![[SRD-barboach-HomeSprite.png]]

*Whiskers Pokémon*
*Their whiskers work as a super sensitive radar. They hide inside the mud, leaving only their whiskers exposed while waiting for prey to come. If the mud dries, they move back into the water.*

**DexID**: 0339
**Name**: Barboach
**Type**: Water / Ground
**Abilities**: [[SRD-Oblivious|Oblivious]] / [[SRD-Anticipation|Anticipation]] ([[SRD-Hydration|Hydration]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⭘⭘  | 1/3  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⭘⭘   | 1/3   |

**Height**: 1'3" / 0.4m
**Weight**: 5.0lbs / 2.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Starter

| Evolves   | Pokemon          | Kind   | Speed   |
|:----------|:-----------------|:-------|:--------|
| To        | [[SRD-Whiscash]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets]]