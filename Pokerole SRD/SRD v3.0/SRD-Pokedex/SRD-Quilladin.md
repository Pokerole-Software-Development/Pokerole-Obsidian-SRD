---
Ability1: Overgrow
Ability2: Bulletproof
AceMoves:
- Iron Defense
- Grass Pledge
- Drain Punch
AdvancedMoves:
- Take Down
- Seed Bomb
- Mud Shot
- Bulk Up
- Body Slam
AmateurMoves: []
BaseHP: 4
BeginnerMoves: []
BookSprite: SRD-quilladin-BookSprite.png
DexCategory: "Spiny Armor Pok\xE9mon"
DexDescription: "It strengthens its lower body by running into hard things. It is\
  \ a kind Pok\xE9mon that relies on its sturdy shell and sharp quills to deflect\
  \ any foe trying to attack it. These easygoing Pok\xE9mon never start a fight."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Chespin]]'
  Speed: Medium
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Chesnaught]]'
  Speed: Medium
ExpertMoves:
- Pain Split
- Wood Hammer
GenderType: ''
GoodStarter: false
Height:
  Feet: 2.3
  Meters: 0.7
HiddenAbility: ''
HomeSprite: SRD-quilladin-HomeSprite.png
Image: quilladin.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 4
MaxStrength: 5
MaxVitality: 6
Name: Quilladin
Number: 651
ProMoves: []
RecommendedRank: Standard
RookieMoves:
- Vine Whip
- Rollout
Special: 2
StandardMoves:
- Bite
- Leech Seed
- Pin Missile
- Needle Arm
StarterMoves:
- Tackle
- Growl
Strength: 2
Type1: Grass
Type2: ''
Vitality: 3
Weight:
  Kilograms: 29.0
  Pounds: 63.0
---

#PokeroleSRD/Pokedex

# Quilladin 

> [!grid]
> ![[SRD-quilladin-BookSprite.png|wsmall]]
> ![[SRD-quilladin-HomeSprite.png]]

*Spiny Armor Pokémon*
*It strengthens its lower body by running into hard things. It is a kind Pokémon that relies on its sturdy shell and sharp quills to deflect any foe trying to attack it. These easygoing Pokémon never start a fight.*

**DexID**: 0651
**Name**: Quilladin
**Type**: Grass
**Abilities**: [[SRD-Overgrow|Overgrow]] / [[SRD-Bulletproof|Bulletproof]]
**Base HP**: 4

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⬤⭘⭘⭘  | 3/6  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 2'3" / 0.7m
**Weight**: 63.0lbs / 29.0kg
**Good Starter**:: No
**Recommended Rank**:: Standard

| Evolves   | Pokemon            | Kind   | Speed   |
|:----------|:-------------------|:-------|:--------|
| From      | [[SRD-Chespin]]    | Level  | Medium  |
| To        | [[SRD-Chesnaught]] | Level  | Medium  |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]