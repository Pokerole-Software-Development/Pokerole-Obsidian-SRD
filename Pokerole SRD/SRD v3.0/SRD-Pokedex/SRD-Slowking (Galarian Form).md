---
Ability1: Curious Medicine
Ability2: Regenerator
AceMoves:
- Hex
- Expanding Force
- Sludge Wave
AdvancedMoves:
- Water Pulse
- Headbutt
- Zen Headbutt
- Nasty Plot
- Swagger
- Amnesia
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-slowking-galarian-form-BookSprite.png
DexCategory: "Royal Pok\xE9mon"
DexDescription: Shellder's powerful toxin enhanced Slowking's intelligence and psychic
  abilities. However, its mind is now controlled by the Shellder. It mutters enchantments
  in a strange tongue and creates strange potions to drink.
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Special
  Pokemon: '[[SRD-Slowpoke (Galarian Form)]]'
  Special: Wear Shellder as a Hat
ExpertMoves:
- Psychic
- Slack Off
- Surf
- Psych Up
- Rain Dance
- Heal Pulse
GenderType: ''
GoodStarter: false
Height:
  Feet: 6.0
  Meters: 1.8
HiddenAbility: ''
HomeSprite: SRD-slowking-galarian-form-HomeSprite.png
Image: slowking-galarian-form.png
Insight: 3
Legendary: false
MaxDexterity: 3
MaxInsight: 6
MaxSpecial: 6
MaxStrength: 4
MaxVitality: 5
Name: Slowking (Galarian Form)
Number: 199
ProMoves: []
RecommendedRank: Advanced
RookieMoves:
- Growl
- Acid
Special: 3
StandardMoves:
- Confusion
- Power Gem
- Yawn
- Curse
- Disable
StarterMoves:
- Eerie Spell
- Tackle
Strength: 2
Type1: Poison
Type2: Psychic
Vitality: 2
Weight:
  Kilograms: 80.0
  Pounds: 175.0
---

#PokeroleSRD/Pokedex

# Slowking (Galarian Form) 

> [!grid]
> ![[SRD-slowking-galarian-form-BookSprite.png|wsmall]]
> ![[SRD-slowking-galarian-form-HomeSprite.png]]

*Royal Pokémon*
*Shellder's powerful toxin enhanced Slowking's intelligence and psychic abilities. However, its mind is now controlled by the Shellder. It mutters enchantments in a strange tongue and creates strange potions to drink.*

**DexID**: 0199G
**Name**: Slowking (Galarian Form)
**Type**: Poison / Psychic
**Abilities**: [[SRD-Curious Medicine|Curious Medicine]] / [[SRD-Regenerator|Regenerator]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⭘⭘ | 1/3 |
| Vitality  | ⬤⬤⭘⭘⭘  | 2/5  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 6'0" / 1.8m
**Weight**: 175.0lbs / 80.0kg
**Good Starter**:: No
**Recommended Rank**:: Advanced

| Evolves   | Pokemon                          | Kind    | Special                |
|:----------|:---------------------------------|:--------|:-----------------------|
| From      | [[SRD-Slowpoke (Galarian Form)]] | Special | Wear Shellder as a Hat |


## Learnset

![[Embedded Views.base#Learnsets]]