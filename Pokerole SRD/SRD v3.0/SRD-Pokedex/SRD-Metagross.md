---
Ability1: Clear Body
Ability2: Light Metal
AceMoves:
- Meteor Beam
- Steel Beam
- Self Destruct
- Expanding Force
AdvancedMoves:
- Magnet Rise
- Zen Headbutt
- Bullet Punch
- Scary Face
- Agility
- Flash Cannon
- Take Down
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-metagross-BookSprite.png
DexCategory: "Iron Leg Pok\xE9mon"
DexDescription: "It has four powerful brains that allow it to react at the speed of\
  \ a super computer. It pins its prey under its massive body, then eats it using\
  \ the large mouth on its stomach. A powerful and ruthless Pok\xE9mon."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Level
  Pokemon: '[[SRD-Metang]]'
  Speed: Slow
- Evolves: To
  Item: Metagrossite
  Kind: Mega
  Pokemon: '[[SRD-Metagross (Mega Form)]]'
ExpertMoves:
- Psychic
- Iron Defense
- Hammer Arm
- Meteor Mash
- Hyper Beam
GenderType: N
GoodStarter: false
Height:
  Feet: 5.3
  Meters: 1.6
HiddenAbility: ''
HomeSprite: SRD-metagross-HomeSprite.png
Image: metagross.png
Insight: 2
Legendary: false
MaxDexterity: 5
MaxInsight: 5
MaxSpecial: 6
MaxStrength: 7
MaxVitality: 7
Name: Metagross
Number: 376
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Confusion
- Metal Claw
Special: 3
StandardMoves:
- Miracle Eye
- Block
- Telekinesis
- Pursuit
StarterMoves:
- Tackle
Strength: 3
Type1: Steel
Type2: Psychic
Vitality: 3
Weight:
  Kilograms: 550.0
  Pounds: 1212.0
---

#PokeroleSRD/Pokedex

# Metagross 

> [!grid]
> ![[SRD-metagross-BookSprite.png|wsmall]]
> ![[SRD-metagross-HomeSprite.png]]

*Iron Leg Pokémon*
*It has four powerful brains that allow it to react at the speed of a super computer. It pins its prey under its massive body, then eats it using the large mouth on its stomach. A powerful and ruthless Pokémon.*

**DexID**: 0376
**Name**: Metagross
**Type**: Steel / Psychic
**Abilities**: [[SRD-Clear Body|Clear Body]] / [[SRD-Light Metal|Light Metal]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Dexterity | ⬤⬤⭘⭘⭘ | 2/5 |
| Vitality  | ⬤⬤⬤⭘⭘⭘⭘  | 3/7  |
| Special   | ⬤⬤⬤⭘⭘⭘   | 3/6   |
| Insight   | ⬤⬤⭘⭘⭘   | 2/5   |

**Height**: 5'3" / 1.6m
**Weight**: 1212.0lbs / 550.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon                       | Kind   | Speed   | Item         |
|:----------|:------------------------------|:-------|:--------|:-------------|
| From      | [[SRD-Metang]]                | Level  | Slow    |              |
| To        | [[SRD-Metagross (Mega Form)]] | Mega   |         | Metagrossite |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]