---
Ability1: Pressure
Ability2: Telepathy
AceMoves: []
AdvancedMoves: []
AmateurMoves: []
BaseHP: 6
BeginnerMoves: []
BookSprite: SRD-dialga-BookSprite.png
DexCategory: "Pok\xE9dex has no data."
DexDescription: In some religions there is a being called 'The God of Time' whose
  first roar brought future, present, and past.
Dexterity: 5
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Form
  Pokemon: '[[SRD-Dialga (Origin Form)]]'
ExpertMoves: []
GenderType: N
GoodStarter: false
Height:
  Feet: 17.8
  Meters: 5.4
HiddenAbility: ''
HomeSprite: SRD-dialga-HomeSprite.png
Image: dialga.png
Insight: 6
Legendary: true
MaxDexterity: 5
MaxInsight: 6
MaxSpecial: 8
MaxStrength: 7
MaxVitality: 7
Name: Dialga
Number: 483
ProMoves: []
RecommendedRank: Champion
RookieMoves: []
Special: 8
StandardMoves: []
StarterMoves: []
Strength: 7
Type1: Steel
Type2: Dragon
Vitality: 7
Weight:
  Kilograms: 683.0
  Pounds: 1505.0
---

#PokeroleSRD/Pokedex

# Dialga 

> [!grid]
> ![[SRD-dialga-BookSprite.png|wsmall]]
> ![[SRD-dialga-HomeSprite.png]]

*Pokédex has no data.*
*In some religions there is a being called 'The God of Time' whose first roar brought future, present, and past.*

**DexID**: 0483
**Name**: Dialga
**Type**: Steel / Dragon
**Abilities**: [[SRD-Pressure|Pressure]] / [[SRD-Telepathy|Telepathy]]
**Base HP**: 6

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⬤⬤⬤⬤  | 7/7  |
| Dexterity | ⬤⬤⬤⬤⬤ | 5/5 |
| Vitality  | ⬤⬤⬤⬤⬤⬤⬤  | 7/7  |
| Special   | ⬤⬤⬤⬤⬤⬤⬤⬤   | 8/8   |
| Insight   | ⬤⬤⬤⬤⬤⬤   | 6/6   |

**Height**: 17'8" / 5.4m
**Weight**: 1505.0lbs / 683.0kg
**Good Starter**:: No
**Recommended Rank**:: Champion

| Evolves   | Pokemon                      | Kind   |
|:----------|:-----------------------------|:-------|
| To        | [[SRD-Dialga (Origin Form)]] | Form   |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]