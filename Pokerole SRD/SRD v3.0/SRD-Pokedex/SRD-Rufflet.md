---
Ability1: Keen Eye
Ability2: Hustle
AceMoves:
- Dual Wingbeat
- Strength
- Heat Wave
AdvancedMoves:
- Air Slash
- Slash
- Defog
- Sky Drop
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-rufflet-BookSprite.png
DexCategory: "Eaglet Pok\xE9mon"
DexDescription: This species only has males. They are independent from the moment
  they hatch. They will challenge even strong opponents without fear. Their frequent
  fights help them become stronger.
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Braviary]]'
  Speed: Slow
- Evolves: To
  Kind: Level
  Pokemon: '[[SRD-Braviary (Hisuian Form)]]'
  Region: Hisui
  Speed: Slow
ExpertMoves:
- Superpower
- Brave Bird
GenderType: M
GoodStarter: true
Height:
  Feet: 1.7
  Meters: 0.5
HiddenAbility: ''
HomeSprite: SRD-rufflet-HomeSprite.png
Image: rufflet.png
Insight: 2
Legendary: false
MaxDexterity: 4
MaxInsight: 4
MaxSpecial: 3
MaxStrength: 5
MaxVitality: 4
Name: Rufflet
Number: 627
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Hone Claws
- Wing Attack
Special: 1
StandardMoves:
- Tailwind
- Scary Face
- Aerial Ace
- Fury Attack
- Whirlwind
- Crush Claw
StarterMoves:
- Peck
- Leer
Strength: 2
Type1: Normal
Type2: Flying
Vitality: 2
Weight:
  Kilograms: 10.0
  Pounds: 23.0
---

#PokeroleSRD/Pokedex

# Rufflet 

> [!grid]
> ![[SRD-rufflet-BookSprite.png|wsmall]]
> ![[SRD-rufflet-HomeSprite.png]]

*Eaglet Pokémon*
*This species only has males. They are independent from the moment they hatch. They will challenge even strong opponents without fear. Their frequent fights help them become stronger.*

**DexID**: 0627
**Name**: Rufflet
**Type**: Normal / Flying
**Abilities**: [[SRD-Keen Eye|Keen Eye]] / [[SRD-Hustle|Hustle]]
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘⭘  | 2/5  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⬤⭘⭘   | 2/4   |

**Height**: 1'7" / 0.5m
**Weight**: 23.0lbs / 10.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon                         | Kind   | Speed   | Region   |
|:----------|:--------------------------------|:-------|:--------|:---------|
| To        | [[SRD-Braviary]]                | Level  | Slow    |          |
| To        | [[SRD-Braviary (Hisuian Form)]] | Level  | Slow    | Hisui    |


## Learnset

![[Embedded Views.base#Learnsets]]