---
Ability1: Oblivious
Ability2: Own Tempo
AceMoves:
- Icy Wind
- Future Sight
- Mud Sport
AdvancedMoves:
- Slack Off
- Amnesia
- Surf
- Psychic
- Psych Up
- Rain Dance
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-slowpoke-BookSprite.png
DexCategory: "Dopey Pok\xE9mon"
DexDescription: "It lives close to water. This Pok\xE9mon has a low intellect, and\
  \ it's slow to react to any stimuli. Its tail seeps a sweet substance it uses to\
  \ lure prey to eat. When the tip turns white, Shellder will be attracted to it."
Dexterity: 1
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Special
  Pokemon: '[[SRD-Slowbro]]'
  Special: Shellder Biting Tail
- Evolves: To
  Item: King's Rock
  Kind: Trade
  Pokemon: '[[SRD-Slowking]]'
ExpertMoves:
- Heal Pulse
GenderType: ''
GoodStarter: true
Height:
  Feet: 4.0
  Meters: 1.2
HiddenAbility: Regenerator
HomeSprite: SRD-slowpoke-HomeSprite.png
Image: slowpoke.png
Insight: 1
Legendary: false
MaxDexterity: 2
MaxInsight: 3
MaxSpecial: 3
MaxStrength: 4
MaxVitality: 4
Name: Slowpoke
Number: 79
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Curse
- Growl
Special: 1
StandardMoves:
- Water Gun
- Confusion
- Disable
- Headbutt
- Water Pulse
- Zen Headbutt
StarterMoves:
- Tackle
- Yawn
Strength: 2
Type1: Water
Type2: Psychic
Vitality: 2
Weight:
  Kilograms: 36.0
  Pounds: 80.0
---

#PokeroleSRD/Pokedex

# Slowpoke 

> [!grid]
> ![[SRD-slowpoke-BookSprite.png|wsmall]]
> ![[SRD-slowpoke-HomeSprite.png]]

*Dopey Pokémon*
*It lives close to water. This Pokémon has a low intellect, and it's slow to react to any stimuli. Its tail seeps a sweet substance it uses to lure prey to eat. When the tip turns white, Shellder will be attracted to it.*

**DexID**: 0079
**Name**: Slowpoke
**Type**: Water / Psychic
**Abilities**: [[SRD-Oblivious|Oblivious]] / [[SRD-Own Tempo|Own Tempo]] ([[SRD-Regenerator|Regenerator]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⭘ | 1/2 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⭘⭘   | 1/3   |
| Insight   | ⬤⭘⭘   | 1/3   |

**Height**: 4'0" / 1.2m
**Weight**: 80.0lbs / 36.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon          | Kind    | Special              | Item        |
|:----------|:-----------------|:--------|:---------------------|:------------|
| To        | [[SRD-Slowbro]]  | Special | Shellder Biting Tail |             |
| To        | [[SRD-Slowking]] | Trade   |                      | King's Rock |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]