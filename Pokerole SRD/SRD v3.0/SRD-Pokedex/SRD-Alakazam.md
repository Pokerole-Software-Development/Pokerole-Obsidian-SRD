---
Ability1: Synchronize
Ability2: Magic Guard
AceMoves:
- Snatch
- Expanding Force
- Foul Play
AdvancedMoves:
- Reflect
- Telekinesis
- Role Play
- Psycho Cut
- Recover
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-alakazam-BookSprite.png
DexCategory: "Psi Pok\xE9mon"
DexDescription: "A Pok\xE9mon that can memorize anything and never forgets what it\
  \ learns. Over time it becomes too smart to allow anyone to be its master. Alakazam's\
  \ psychic powers can be terrifying."
Dexterity: 3
EventAbilities: ''
Evolutions:
- Evolves: From
  Kind: Trade
  Pokemon: '[[SRD-Kadabra]]'
- Evolves: To
  Item: Alakazite
  Kind: Mega
  Pokemon: '[[SRD-Alakazam (Mega Form)]]'
ExpertMoves:
- Psyshock
- Psychic
- Future Sight
- Calm Mind
- Wonder Room
GenderType: ''
GoodStarter: false
Height:
  Feet: 5.0
  Meters: 1.5
HiddenAbility: ''
HomeSprite: SRD-alakazam-HomeSprite.png
Image: alakazam.png
Insight: 3
Legendary: false
MaxDexterity: 7
MaxInsight: 6
MaxSpecial: 7
MaxStrength: 4
MaxVitality: 4
Name: Alakazam
Number: 65
ProMoves: []
RecommendedRank: Expert
RookieMoves:
- Confusion
- Disable
Special: 3
StandardMoves:
- Miracle Eye
- Ally Switch
- Psybeam
StarterMoves:
- Teleport
- Kinesis
Strength: 2
Type1: Psychic
Type2: ''
Vitality: 2
Weight:
  Kilograms: 48.0
  Pounds: 105.0
---

#PokeroleSRD/Pokedex

# Alakazam 

> [!grid]
> ![[SRD-alakazam-BookSprite.png|wsmall]]
> ![[SRD-alakazam-HomeSprite.png]]

*Psi Pokémon*
*A Pokémon that can memorize anything and never forgets what it learns. Over time it becomes too smart to allow anyone to be its master. Alakazam's psychic powers can be terrifying.*

**DexID**: 0065
**Name**: Alakazam
**Type**: Psychic
**Abilities**: [[SRD-Synchronize|Synchronize]] / [[SRD-Magic Guard|Magic Guard]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⭘⭘  | 2/4  |
| Dexterity | ⬤⬤⬤⭘⭘⭘⭘ | 3/7 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |
| Insight   | ⬤⬤⬤⭘⭘⭘   | 3/6   |

**Height**: 5'0" / 1.5m
**Weight**: 105.0lbs / 48.0kg
**Good Starter**:: No
**Recommended Rank**:: Expert

| Evolves   | Pokemon                      | Kind   | Item      |
|:----------|:-----------------------------|:-------|:----------|
| From      | [[SRD-Kadabra]]              | Trade  |           |
| To        | [[SRD-Alakazam (Mega Form)]] | Mega   | Alakazite |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]