---
Ability1: Berserk
Ability2: ''
AceMoves:
- Gust
- Leer
- Payback
- Safeguard
- Wing Attack
- Agility
- Ancient Power
- Sucker Punch
- Air Slash
- After You
- Fiery Wrath
- Nasty Plot
- Hurricane
- Endure
- Memento
- Sky Attack
- Dark Pulse
- Hyper Voice
- Hex
AdvancedMoves: []
AmateurMoves: []
BaseHP: 5
BeginnerMoves: []
BookSprite: SRD-moltres-galarian-form-BookSprite.png
DexCategory: "Pok\xE9dex has no Data."
DexDescription: "Folktales speak of a horrendous black bird whose evil aura burns\
  \ the spirit within. Leaving its poor victims alive but empty shells of their former\
  \ selves. If such pok\xE9mon exists it should be avoided at all costs."
Dexterity: 5
EventAbilities: ''
Evolutions: []
ExpertMoves: []
GenderType: N
GoodStarter: false
Height:
  Feet: 6.6
  Meters: 2.0
HiddenAbility: ''
HomeSprite: SRD-moltres-galarian-form-HomeSprite.png
Image: moltres-galarian-form.png
Insight: 7
Legendary: true
MaxDexterity: 5
MaxInsight: 7
MaxSpecial: 6
MaxStrength: 5
MaxVitality: 5
Name: Moltres (Galarian Form)
Number: 146
ProMoves: []
RecommendedRank: Ace
RookieMoves: []
Special: 6
StandardMoves: []
StarterMoves: []
Strength: 5
Type1: Dark
Type2: Flying
Vitality: 5
Weight:
  Kilograms: 66.0
  Pounds: 145.0
---

#PokeroleSRD/Pokedex

# Moltres (Galarian Form) 

> [!grid]
> ![[SRD-moltres-galarian-form-BookSprite.png|wsmall]]
> ![[SRD-moltres-galarian-form-HomeSprite.png]]

*Pokédex has no Data.*
*Folktales speak of a horrendous black bird whose evil aura burns the spirit within. Leaving its poor victims alive but empty shells of their former selves. If such pokémon exists it should be avoided at all costs.*

**DexID**: 0146G
**Name**: Moltres (Galarian Form)
**Type**: Dark / Flying
**Abilities**: [[SRD-Berserk|Berserk]]
**Base HP**: 5

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⬤⬤⬤⬤  | 5/5  |
| Dexterity | ⬤⬤⬤⬤⬤ | 5/5 |
| Vitality  | ⬤⬤⬤⬤⬤  | 5/5  |
| Special   | ⬤⬤⬤⬤⬤⬤   | 6/6   |
| Insight   | ⬤⬤⬤⬤⬤⬤⬤   | 7/7   |

**Height**: 6'6" / 2.0m
**Weight**: 145.0lbs / 66.0kg
**Good Starter**:: No
**Recommended Rank**:: Ace


## Learnset

![[Embedded Views.base#Learnsets]]