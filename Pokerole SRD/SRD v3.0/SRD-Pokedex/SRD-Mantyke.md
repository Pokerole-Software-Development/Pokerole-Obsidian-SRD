---
Ability1: Swift Swim
Ability2: Water Absorb
AceMoves:
- Twister
- Helping Hand
- Tailwind
AdvancedMoves:
- Agility
- Air Slash
- Aqua Ring
- Bounce
AmateurMoves: []
BaseHP: 3
BeginnerMoves: []
BookSprite: SRD-mantyke-BookSprite.png
DexCategory: "Kite Pok\xE9mon"
DexDescription: "When it swims close the ocean's surface people aboard ships are able\
  \ to observe the pattern on its back as it is different in every region. Mantyke\
  \ is an intelligent and friendly Pok\xE9mon that rarely attacks others."
Dexterity: 2
EventAbilities: ''
Evolutions:
- Evolves: To
  Kind: Special
  Pokemon: '[[SRD-Mantine]]'
  Special: Bit by Remoraid
ExpertMoves:
- Hydro Pump
GenderType: ''
GoodStarter: true
Height:
  Feet: 3.3
  Meters: 1.0
HiddenAbility: Water Veil
HomeSprite: SRD-mantyke-HomeSprite.png
Image: mantyke.png
Insight: 3
Legendary: false
MaxDexterity: 4
MaxInsight: 7
MaxSpecial: 4
MaxStrength: 3
MaxVitality: 4
Name: Mantyke
Number: 458
ProMoves: []
RecommendedRank: Rookie
RookieMoves:
- Supersonic
- Bubble Beam
Special: 2
StandardMoves:
- Confuse Ray
- Wing Attack
- Headbutt
- Water Pulse
- Take Down
- Wide Guard
StarterMoves:
- Tackle
- Bubble
Strength: 1
Type1: Water
Type2: Flying
Vitality: 2
Weight:
  Kilograms: 65.0
  Pounds: 143.0
---

#PokeroleSRD/Pokedex

# Mantyke 

> [!grid]
> ![[SRD-mantyke-BookSprite.png|wsmall]]
> ![[SRD-mantyke-HomeSprite.png]]

*Kite Pokémon*
*When it swims close the ocean's surface people aboard ships are able to observe the pattern on its back as it is different in every region. Mantyke is an intelligent and friendly Pokémon that rarely attacks others.*

**DexID**: 0458
**Name**: Mantyke
**Type**: Water / Flying
**Abilities**: [[SRD-Swift Swim|Swift Swim]] / [[SRD-Water Absorb|Water Absorb]] ([[SRD-Water Veil|Water Veil]])
**Base HP**: 3

|           |                 |                |
| --------- | --------------- | -------------- |
| Strength  | ⬤⭘⭘  | 1/3  |
| Dexterity | ⬤⬤⭘⭘ | 2/4 |
| Vitality  | ⬤⬤⭘⭘  | 2/4  |
| Special   | ⬤⬤⭘⭘   | 2/4   |
| Insight   | ⬤⬤⬤⭘⭘⭘⭘   | 3/7   |

**Height**: 3'3" / 1.0m
**Weight**: 143.0lbs / 65.0kg
**Good Starter**:: Yes
**Recommended Rank**:: Rookie

| Evolves   | Pokemon         | Kind    | Special         |
|:----------|:----------------|:--------|:----------------|
| To        | [[SRD-Mantine]] | Special | Bit by Remoraid |


## Learnset

![[Embedded Views.base#Learnsets v3.0]]